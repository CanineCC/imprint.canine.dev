...
`;function s(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function f(t){if(t==null||t==="")return"";let n=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,e="",a=0,i;for(;(i=n.exec(t))!==null;){i.index>a&&(e+=s(t.slice(a,i.index)));let o=i[0];if(o.startsWith("**"))e+=`<strong>${s(o.slice(2,-2))}</strong>`;else if(o.startsWith("`"))e+=`<code>${s(o.slice(1,-1))}</code>`;else{let l=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(o);l?e+=`<a href="${s(l[2])}">${s(l[1])}</a>`:e+=s(o)}a=i.index+o.length}return a<t.length&&(e+=s(t.slice(a))),e}var L=`
...
`;function O(t){let n=t.getAttribute("kicker"),e=t.getAttribute("heading"),a=t.getAttribute("lede");if(!n&&!e&&!a)return"";let i='<div class="mk-section-head">';return n&&(i+=`<span class="mk-kicker">${s(n)}</span>`),e&&(i+=`<h2>${f(e)}</h2>`),a&&(i+=`<p>${f(a)}</p>`),i+="</div>",i}var R=`
...
`;function B(t,{right:n=!1,label:e="More information"}={}){let a=t==null?"":String(t).trim();if(a==="")return"";let i=a.split(/\n\s*\n/).map(l=>l.trim()).filter(l=>l!=="").map(l=>`<p>${f(l)}</p>`).join("");return`<span class="${n?"info-hint hint-right":"info-hint"}" tabindex="0" role="note" aria-label="${s(e||"More information")}"><span class="info-hint-dot" aria-hidden="true">i</span><span class="info-hint-tip">${i}</span></span>`}var W=`
...
`;function _({kicker:t,tip:n,content:e}){return`<div class="rail"><div class="rail-side"><span class="rail-kicker">${s(t)}</span>`+B(n,{label:t})+`</div><div class="rail-body">${e}</div></div>`}function q(t){let n=t.getAttribute("heading"),e=t.getAttribute("lede");if(!n&&!e)return"";let a='<div class="mk-section-head">';return n&&(a+=`<h2>${f(n)}</h2>`),e&&(a+=`<p>${f(e)}</p>`),a+="</div>",a}var y=[0,25,50,70,90,100],P=.05;function K(t){let n=[],e=0;for(;e<t.length;){let a=e;for(;a+1<t.length&&Math.abs(t[a+1]-t[e])<P;)a++;let i=a-e+1;n.push({i:e,v:t[e],run:i}),a>e&&n.push({i:a,v:t[a],run:i}),e=a+1}return n}var C=720,H=240,h={top:26,right:18,bottom:34,left:40},X=z+L+R+j+D+W+`
...
`;function J(t){let n=Math.min(...t),e=Math.max(...t),a=0,i=100;for(let o of y)o<=n&&(a=o);for(let o=y.length-1;o>=0;o--)y[o]>=e&&(i=y[o]);return i-a<10&&(a=Math.max(0,Math.min(a,i-25))),i-a<10&&(i=Math.min(100,a+25)),{min:a,max:i}}function w(t){return(Math.round(t*10)/10).toFixed(1)}customElements.define("cai-trend",class extends T{render(t){let n=(this.json("series",[])||[]).map(Number).filter(d=>Number.isFinite(d)),e=(this.getAttribute("kicker")||"").trim(),a=this.figuresHtml(),i=this.plotHtml(n),o=a?`<div class="mk-trend-row">${i}${a}</div>`:i,l=`<style>${X}</style>`;l+=e?_({kicker:e,tip:this.getAttribute("tip"),content:q(this)+o}):O(this)+o,t.innerHTML=l,n.length>1&&this.wireTips(t,n)}figuresHtml(){let t=(this.json("figures",[])||[]).filter(e=>e&&e.from!=null&&e.from.value!=null&&String(e.from.value)!==""&&e.to!=null&&e.to.value!=null&&String(e.to.value)!=="");if(t.length===0)return"";let n='<div class="mk-trend-figs">';for(let e of t){n+='<div class="mk-trend-fig">',e.label!=null&&String(e.label)!==""&&(n+=`<span class="mk-trend-fig-label">${s(String(e.label))}</span>`),n+=`<span class="mk-trend-fig-pair">${s(String(e.from.value))} \u2192 ${s(String(e.to.value))}</span>`;let a=e.from.sub==null?"":String(e.from.sub),i=e.to.sub==null?"":String(e.to.sub),o=a===i?[a]:[a,i];for(let l of o.filter(d=>d!==""))n+=`<span class="mk-trend-fig-sub">${s(l)}</span>`;n+="</div>"}return n+"</div>"}plotHtml(t){let n=this.getAttribute("first-date"),e=this.getAttribute("last-date"),a=this.getAttribute("caption"),i=(this.getAttribute("sampled")||"").trim();if(t.length===0)return"";if(t.length===1){let r=t[0],u=E(r),g='<div class="mk-trend"><p class="mk-trend-solo">';return g+=`<span class="mk-trend-solo-num ink-${u.key}">${w(r)}</span>`,(e||n)&&(g+=`<span class="mk-trend-solo-date">measured ${s(e||n)}</span>`),g+="</p></div>",a&&(g+=`<p class="mk-trend-sum">${f(a)}</p>`),g}let{min:o,max:l}=J(t),d=C-h.left-h.right,$=H-h.top-h.bottom,m=r=>h.left+d*r/(t.length-1),p=r=>h.top+$*(1-(r-o)/(l-o)),b=t[t.length-1],S=E(b),A=i?t.map((r,u)=>({i:u,v:r,run:1})):K(t),k=A.map(r=>`${m(r.i).toFixed(1)},${p(r.v).toFixed(1)}`),c=`<div class="mk-trend"><div class="mk-trend-plot"${i?` data-sampled="${s(i)}"`:""}>`,N=i?`${t.length} ${i} samples`:`${t.length} measurements`,x=[w(t[0]),w(b)],I=(this.json("figures",[])||[]).some(r=>r&&r.from&&r.to&&String(r.from.value??"").trim()===x[0]&&String(r.to.value??"").trim()===x[1])&&!!n&&!!e;c+=`<svg viewBox="0 0 ${C} ${H}" role="img" aria-label="${s(I?`${N}, from ${n} to ${e}: ${x[0]} to ${x[1]}.`:`${N}, from ${x[0]} to ${x[1]}.`)}">`;for(let r of y){if(r<o||r>l)continue;let u=p(r).toFixed(1);c+=`<line class="mk-trend-grid" x1="${h.left}" y1="${u}" x2="${C-h.right}" y2="${u}"></line>`,c+=`<text class="mk-trend-cut" x="${h.left-8}" y="${u}" text-anchor="end" dominant-baseline="middle">${r}</text>`}c+=`<path class="mk-trend-area" d="M${m(0).toFixed(1)},${p(o).toFixed(1)} L${k.join(" L")} L${m(t.length-1).toFixed(1)},${p(o).toFixed(1)} Z"></path>`,c+=`<polyline class="mk-trend-line" points="${k.join(" ")}" vector-effect="non-scaling-stroke"></polyline>`,n&&(c+=`<text class="mk-trend-date" x="${h.left}" y="${H-10}" text-anchor="start">${s(n)}</text>`),e&&(c+=`<text class="mk-trend-date" x="${C-h.right}" y="${H-10}" text-anchor="end">${s(e)}</text>`),A.forEach(r=>{let u=r.i===t.length-1,g=m(r.i).toFixed(1),F=p(r.v).toFixed(1);c+=`<circle class="mk-trend-hit" cx="${g}" cy="${F}" r="18" tabindex="0" data-i="${r.i}" data-v="${w(r.v)}" data-run="${r.run}"></circle>`,c+=u?`<circle class="mk-trend-end fill-${S.key}" cx="${g}" cy="${F}" r="5.5"></circle>`:`<circle class="mk-trend-dot" cx="${g}" cy="${F}" r="4"></circle>`}),c+=`<text class="mk-trend-endlabel ink-${S.key}" x="${m(t.length-1).toFixed(1)}" y="${(p(b)-14).toFixed(1)}" text-anchor="end">${w(b)}</text>`,c+="</svg>",c+='<div class="mk-trend-tip" hidden></div>',c+="</div>";let M=b-t[0],U=Math.abs(M)<.05?"unchanged":`${M>0?"up":"down"} ${w(Math.abs(M))}`,G=`${n?`, from ${n}`:""}${e?` to ${e}`:""}: ${x[0]} to ${x[1]} \u2014 ${U}.`;return c+=`<p class="mk-trend-sum">${s(N+(I?".":G)+(i?" Each point is the newest reading on or before its own date, not a reading taken on it.":""))}</p>`,a&&(c+=`<p class="mk-trend-sum">${f(a)}</p>`),c+="</div>",c}wireTips(t,n){let e=t.querySelector(".mk-trend-tip"),a=t.querySelector(".mk-trend-plot");if(!e||!a)return;let i=a.getAttribute("data-sampled")||"",o=d=>{let $=Number(d.getAttribute("data-i")),m=d.getBoundingClientRect(),p=a.getBoundingClientRect(),b=Number(d.getAttribute("data-run"))||1,S=i?`${i} sample ${$+1} of ${n.length}`:b>=3?`unchanged across ${b} scans`:`scan ${$+1} of ${n.length}`;e.hidden=!1,e.innerHTML=`<b>${s(d.getAttribute("data-v")||"")}</b> \xB7 ${s(S)}`+(i?'<span class="mk-trend-tip-note">the newest reading on or before that date</span>':"");let A=m.left+m.width/2-p.left,k=e.offsetWidth/2,c=Math.min(Math.max(A-k,0),Math.max(0,p.width-e.offsetWidth));e.style.left=`${c+k}px`,e.style.top=`${m.top-p.top-6}px`,e.classList.add("on")},l=()=>{e.classList.remove("on")};for(let d of t.querySelectorAll(".mk-trend-hit"))d.addEventListener("pointerenter",()=>o(d)),d.addEventListener("focus",()=>o(d)),d.addEventListener("pointerleave",l),d.addEventListener("blur",l)}});export{K as collapseFlatRuns};
