...
`;function o(n){return String(n??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function c(n){if(n==null||n==="")return"";let i=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,t="",e=0,r;for(;(r=i.exec(n))!==null;){r.index>e&&(t+=o(n.slice(e,r.index)));let s=r[0];if(s.startsWith("**"))t+=`<strong>${o(s.slice(2,-2))}</strong>`;else if(s.startsWith("`"))t+=`<code>${o(s.slice(1,-1))}</code>`;else{let a=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(s);a?t+=`<a href="${o(a[2])}">${o(a[1])}</a>`:t+=o(s)}e=r.index+s.length}return e<n.length&&(t+=o(n.slice(e))),t}var m=`
...
`;function g(n){let i=n.getAttribute("kicker"),t=n.getAttribute("heading"),e=n.getAttribute("lede");if(!i&&!t&&!e)return"";let r='<div class="mk-section-head">';return i&&(r+=`<span class="mk-kicker">${o(i)}</span>`),t&&(r+=`<h2>${c(t)}</h2>`),e&&(r+=`<p>${c(e)}</p>`),r+="</div>",r}var u=`
...
`,d=class extends HTMLElement{#e;connectedCallback(){this.shadowRoot||this.attachShadow({mode:"open"}),this.#t(),this.render(this.shadowRoot),typeof this.liveLoad=="function"&&Promise.resolve(this.liveLoad()).catch(()=>{}),this.#e=new MutationObserver(()=>{let i=this.dataset.theme;this.#t(),this.dataset.theme!==i&&this.render(this.shadowRoot)}),this.#e.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}apiBase(){return(this.getAttribute("api-base")||"").trim()}disconnectedCallback(){this.#e?.disconnect()}#t(){let i=document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.dataset.theme=i;let t=(this.getAttribute("brand")||"").trim().toLowerCase();t==="assay"||t==="cai"||t==="watchdog"?this.dataset.brand=t:delete this.dataset.brand}json(i,t){let e=this.getAttribute(i);if(e==null||e.trim()==="")return t;try{return JSON.parse(e)}catch{return t}}};var b=`
...
`;function h(n,{right:i=!1,label:t="More information"}={}){let e=n==null?"":String(n).trim();if(e==="")return"";let r=e.split(/\n\s*\n/).map(a=>a.trim()).filter(a=>a!=="").map(a=>`<p>${c(a)}</p>`).join("");return`<span class="${i?"info-hint hint-right":"info-hint"}" tabindex="0" role="note" aria-label="${o(t||"More information")}"><span class="info-hint-dot" aria-hidden="true">i</span><span class="info-hint-tip">${r}</span></span>`}var k=`
...
`;function w({kicker:n,tip:i,content:t}){return`<div class="rail"><div class="rail-side"><span class="rail-kicker">${o(n)}</span>`+h(i,{label:n})+`</div><div class="rail-body">${t}</div></div>`}function x(n){let i=n.getAttribute("heading"),t=n.getAttribute("lede");if(!i&&!t)return"";let e='<div class="mk-section-head">';return i&&(e+=`<h2>${c(i)}</h2>`),t&&(e+=`<p>${c(t)}</p>`),e+="</div>",e}var y="/brand/canine-badge.svg",S=f+m+u+b+k+`
...
`,v={github:'<svg viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8Z"/></svg>',gitlab:'<svg viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="m15.73 6.49-.02-.06-2.17-5.66a.57.57 0 0 0-.22-.27.58.58 0 0 0-.88.27L10.98 5.2H5.03L3.57.77a.58.58 0 0 0-.88-.27.57.57 0 0 0-.22.27L.3 6.43l-.02.06a4.03 4.03 0 0 0 1.34 4.65l.01.01.02.01 3.3 2.47 1.63 1.23.99.75a.68.68 0 0 0 .82 0l.99-.75 1.64-1.23 3.32-2.48.01-.01a4.03 4.03 0 0 0 1.34-4.65Z"/></svg>',html:'<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M1.5 0h21l-1.91 21.563L11.977 24l-8.564-2.438L1.5 0zm7.031 9.75l-.232-2.718 10.059.003.23-2.622L5.412 4.41l.698 8.01h9.126l-.326 3.426-2.91.804-2.955-.81-.188-2.11H6.248l.33 4.171L12 19.351l5.379-1.443.744-8.157H8.531z"/></svg>',doc:'<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9.2 1.4H4a1.6 1.6 0 0 0-1.6 1.6v10A1.6 1.6 0 0 0 4 14.6h8a1.6 1.6 0 0 0 1.6-1.6V5.8Z"/><path d="M9.2 1.4v4.4h4.4"/><path d="M5.6 8.4h4.8M5.6 11h4.8"/></svg>'};customElements.define("cai-link-cards",class extends d{render(n){let i=(this.json("links",[])||[]).filter(a=>a&&a.href&&a.label),t=(this.getAttribute("kicker")||"").trim(),e='<div class="mk-links">';for(let a of i){let l=String(a.icon||"").toLowerCase();e+=`<a class="mk-link" href="${o(a.href)}" rel="noopener noreferrer">`,e+='<span class="mk-link-ico" aria-hidden="true">',l==="watchdog"||l==="cai"?e+=`<span class="mk-link-badge is-${l}" style="--badge:url('${y}')"></span>`:e+=v[l]||v.doc,e+="</span>";let p=h(a.tip,{right:!0,label:a.label});e+=p?`<span class="mk-link-label-row"><span class="mk-link-label">${o(a.label)}</span>${p}</span>`:`<span class="mk-link-label">${o(a.label)}</span>`,a.figure!=null&&String(a.figure)!==""&&(e+=`<span class="mk-link-figure">${o(String(a.figure))}</span>`),e+=`<span class="mk-link-note">${o(a.note||"")}</span>`,a.go!=null&&String(a.go)!==""&&(e+=`<span class="mk-link-go">${o(String(a.go))} \u2192</span>`),e+="</a>"}e+="</div>";let r=i.length===0?"":e,s=`<style>${S}</style>`;s+=t?w({kicker:t,tip:this.getAttribute("tip"),content:x(this)+r}):g(this)+r,n.innerHTML=s}});
