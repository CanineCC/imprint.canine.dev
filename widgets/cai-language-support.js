...
`;function s(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function l(e){if(e==null||e==="")return"";let a=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,r="",t=0,i;for(;(i=a.exec(e))!==null;){i.index>t&&(r+=s(e.slice(t,i.index)));let n=i[0];if(n.startsWith("**"))r+=`<strong>${s(n.slice(2,-2))}</strong>`;else if(n.startsWith("`"))r+=`<code>${s(n.slice(1,-1))}</code>`;else{let o=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(n);o?r+=`<a href="${s(o[2])}">${s(o[1])}</a>`:r+=s(n)}t=i.index+n.length}return t<e.length&&(r+=s(e.slice(t))),r}var u=`
...
`;function f(e){let a=e.getAttribute("kicker"),r=e.getAttribute("heading"),t=e.getAttribute("lede");if(!a&&!r&&!t)return"";let i='<div class="mk-section-head">';return a&&(i+=`<span class="mk-kicker">${s(a)}</span>`),r&&(i+=`<h2>${l(r)}</h2>`),t&&(i+=`<p>${l(t)}</p>`),i+="</div>",i}var m=`
...
`;function w(e){let a=x[e]||0,r='<span class="mk-fit-dots" aria-hidden="true">';for(let t=1;t<=4;t++)r+=`<i class="${t<=a?"on":""}"></i>`;return r+"</span>"}function S(e){let a={FULL:1,HIGH:.7,MEDIUM:.45,LOW:.22}[e.band]??.5,r=(e.coveredLenses||[]).map(c=>`<span class="mk-fit-chip">${s(c)}</span>`).join(""),t=(e.notApplicableLenses||[]).map(c=>`<span class="mk-fit-chip na">${s(c)}</span>`).join(""),i=t?`<div class="mk-fit-lensrow"><span class="mk-fit-lenslabel">N/A</span><div class="mk-fit-chips">${t}</div></div>`:"",n=e.signedOffOn?`signed off ${s(e.signedOffOn)}`:"baseline coverage",o=Math.max(0,Math.min(100,(Number(e.applicability)||0)*10));return`<article class="mk-fit-card" style="--edge:${a}"><div class="mk-fit-top"><div><div class="mk-fit-name">${s(e.displayName)}</div><div class="mk-fit-code">${s(e.code)}</div></div><span class="mk-fit-kind">${s(e.supportKind||"")}</span></div><div class="mk-fit-gauge"><div class="mk-fit-track"><div class="mk-fit-fill" style="width:${o}%"></div></div><span class="mk-fit-num">${s(String(e.applicability))}<small>/10</small></span></div><span class="mk-fit-pill"><i></i>${s(e.bandLabel||e.band)} fit</span><p class="mk-fit-summary">${s(e.summary||"")}</p><div class="mk-fit-lenses"><div class="mk-fit-lensrow"><span class="mk-fit-lenslabel">Lenses</span><div class="mk-fit-chips">${r}</div></div>${i}</div><div class="mk-fit-foot">${n}</div></article>`}customElements.define("cai-language-support",class extends d{#e=null;async liveLoad(){let e=this.apiBase();if(!e)return;let a=await h(e);!a||!Array.isArray(a.languages)||a.languages.length===0||(this.#e=a,this._live=!0,this.render(this.shadowRoot))}render(e){let a=this._live&&this.#e?this.#e:y,r={};for(let n of a.bands||[])r[n.band]=n.why;let t=`<style>${k}</style>`;if(t+=f(this),t+='<div class="mk-fit">',a.note&&!this.getAttribute("lede")){let n=a.note.split("\u2014"),o=s(n[0].trim()),c=s(n.slice(1).join("\u2014").trim());t+=`<p class="mk-fit-note"><strong>${o}</strong>${c?" \u2014 "+c:""}</p>`}for(let n of v){let o=a.languages.filter(c=>c.band===n);o.length!==0&&(t+='<section class="mk-fit-band"><div class="mk-fit-bandhead">',t+=`<span class="mk-fit-bandname">${w(n)} ${s(n)} fit</span>`,t+=`<span class="mk-fit-bandcount">${o.length} language${o.length>1?"s":""}</span>`,t+="</div>",r[n]&&(t+=`<p class="mk-fit-why">${s(r[n])}</p>`),t+=`<div class="mk-fit-grid">${o.map(S).join("")}</div>`,t+="</section>")}t+="</div>";let i=this.getAttribute("footnote");i&&(t+=`<p class="mk-grid-foot">${l(i)}</p>`),e.innerHTML=t}});
