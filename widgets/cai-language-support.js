...
`;function r(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function p(e){if(e==null||e==="")return"";let a=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,n="",t=0,o;for(;(o=a.exec(e))!==null;){o.index>t&&(n+=r(e.slice(t,o.index)));let s=o[0];if(s.startsWith("**"))n+=`<strong>${r(s.slice(2,-2))}</strong>`;else if(s.startsWith("`"))n+=`<code>${r(s.slice(1,-1))}</code>`;else{let c=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(s);c?n+=`<a href="${r(c[2])}">${r(c[1])}</a>`:n+=r(s)}t=o.index+s.length}return t<e.length&&(n+=r(e.slice(t))),n}var g=`
...
`;function h(e){let a=e.getAttribute("kicker"),n=e.getAttribute("heading"),t=e.getAttribute("lede");if(!a&&!n&&!t)return"";let o='<div class="mk-section-head">';return a&&(o+=`<span class="mk-kicker">${r(a)}</span>`),n&&(o+=`<h2>${p(n)}</h2>`),t&&(o+=`<p>${p(t)}</p>`),o+="</div>",o}var b=`
...
`,u=class extends HTMLElement{#e;connectedCallback(){this.shadowRoot||this.attachShadow({mode:"open"}),this.#t(),this.render(this.shadowRoot),typeof this.liveLoad=="function"&&Promise.resolve(this.liveLoad()).catch(()=>{}),this.#e=new MutationObserver(()=>{let a=this.dataset.theme;this.#t(),this.dataset.theme!==a&&this.render(this.shadowRoot)}),this.#e.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}apiBase(){return(this.getAttribute("api-base")||"").trim()}disconnectedCallback(){this.#e?.disconnect()}#t(){let a=document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.dataset.theme=a;let n=(this.getAttribute("brand")||"").trim().toLowerCase();n==="assay"||n==="cai"||n==="watchdog"?this.dataset.brand=n:delete this.dataset.brand}json(a,n){let t=this.getAttribute(a);if(t==null||t.trim()==="")return n;try{return JSON.parse(t)}catch{return n}}};var y=new Map;function k(e,a,n){let t=(e||"").trim().replace(/\/$/,"");if(!t)return Promise.resolve(n);let o=t+" "+a,s=y.get(o);return s||(s=(async()=>{try{let c=await fetch(t+a);return c.ok?await c.json():n}catch{return n}})(),y.set(o,s),s)}async function v(e){let a=await k(e,"/api/public/language-support",{languages:[],bands:[]});return a&&Array.isArray(a.languages)?a:{languages:[],bands:[]}}var w={note:"FIT is survey clarity \u2014 how clearly a language leads to a complete architecture survey. It is NOT a measure of language or code quality.",languages:[{code:"csharp",displayName:"C#",applicability:10,supportKind:"Deep",band:"FULL",summary:"Native symbols resolve every lens with full fidelity.",coveredLenses:["DDD","Event sourcing","Event-driven","Vertical slice"],notApplicableLenses:[]},{code:"java",displayName:"Java",applicability:9,supportKind:"Deep",band:"VERY HIGH",summary:"Source-resolution is strong; the full domain survey fires.",coveredLenses:["DDD","Event sourcing","Event-driven","Vertical slice"],notApplicableLenses:["strongly-typed-id (field-based)"]},{code:"python",displayName:"Python",applicability:7,supportKind:"Deep",band:"HIGH",summary:"Marker lenses are strong; call-owner lenses partial on untyped code \u2014 declined, never guessed.",coveredLenses:["DDD","Event sourcing","Event-driven"],notApplicableLenses:["Sealed/DU lens"]},{code:"ruby",displayName:"Ruby",applicability:6,supportKind:"Deep",band:"MEDIUM",summary:"The hardest static target; markers and block-fold DSLs still resolve.",coveredLenses:["DDD","Event sourcing","Event-driven"],notApplicableLenses:["god-class/LCOM"]},{code:"javascript",displayName:"JavaScript",applicability:4,supportKind:"Structural",band:"LOW",summary:"Structural suite only, without types \u2014 the least architecture signal.",coveredLenses:["Structural","Module graph"],notApplicableLenses:["DDD","Event sourcing"]}],bands:[{band:"VERY HIGH",label:"VERY HIGH",why:"Nearly every lens applies and resolves - a very clear survey, with a named lens or two deferred for this language. About survey clarity, not language quality."},{band:"FULL",label:"FULL",why:"Every lens applies and resolves cleanly \u2014 the clearest surveys we produce. About survey clarity, not language quality."},{band:"HIGH",label:"HIGH",why:"Most lenses apply and resolve \u2014 a clear, well-populated survey. About survey clarity, not language quality."},{band:"MEDIUM",label:"MEDIUM",why:"A meaningful slice of the survey is populated; some lenses are N/A or best-effort. About survey clarity, not language quality."},{band:"LOW",label:"LOW",why:"The structural lenses apply but not the deep domain catalogue. What we report is real; there is simply less of it. About survey clarity, not language quality."}]},f=[{band:"FULL",dots:5,edge:1},{band:"VERY HIGH",dots:4,edge:.85},{band:"HIGH",dots:3,edge:.7},{band:"MEDIUM",dots:2,edge:.45},{band:"LOW",dots:1,edge:.22}],x=f.map(e=>e.band),S=Object.fromEntries(f.map(e=>[e.band,e.dots])),A=Object.fromEntries(f.map(e=>[e.band,e.edge])),L=Math.max(...f.map(e=>e.dots)),E=m+g+b+`
...
`;function H(e){let a=S[e]||0,n='<span class="mk-fit-dots" aria-hidden="true">';for(let t=1;t<=L;t++)n+=`<i class="${t<=a?"on":""}"></i>`;return n+"</span>"}function $(e){let a=A[e.band]??.5,n=(e.coveredLenses||[]).map(i=>`<span class="mk-fit-chip">${r(i)}</span>`).join(""),t=(e.notApplicableLenses||[]).map(i=>`<span class="mk-fit-chip na">${r(i)}</span>`).join(""),o=t?`<div class="mk-fit-lensrow"><span class="mk-fit-lenslabel">N/A</span><div class="mk-fit-chips">${t}</div></div>`:"",s=e.signedOffOn?`signed off ${r(e.signedOffOn)}`:"baseline coverage",c=Math.max(0,Math.min(100,(Number(e.applicability)||0)*10));return`<article class="mk-fit-card" style="--edge:${a}"><div class="mk-fit-top"><div><div class="mk-fit-name">${r(e.displayName)}</div><div class="mk-fit-code">${r(e.code)}</div></div><span class="mk-fit-kind">${r(e.supportKind||"")}</span></div><div class="mk-fit-gauge"><div class="mk-fit-track"><div class="mk-fit-fill" style="width:${c}%"></div></div><span class="mk-fit-num">${r(String(e.applicability))}<small>/10</small></span></div><span class="mk-fit-pill"><i></i>${r(e.bandLabel||e.band)} fit</span><p class="mk-fit-summary">${r(e.summary||"")}</p><div class="mk-fit-lenses"><div class="mk-fit-lensrow"><span class="mk-fit-lenslabel">Lenses</span><div class="mk-fit-chips">${n}</div></div>${o}</div><div class="mk-fit-foot">${s}</div></article>`}customElements.define("cai-language-support",class extends u{#e=null;async liveLoad(){let e=this.apiBase();if(!e)return;let a=await v(e);!a||!Array.isArray(a.languages)||a.languages.length===0||(this.#e=a,this._live=!0,this.render(this.shadowRoot))}render(e){let a=this._live&&this.#e?this.#e:w,n={};for(let i of a.bands||[])n[i.band]=i.why;let t=`<style>${E}</style>`;if(t+=h(this),t+='<div class="mk-fit">',a.note&&!this.getAttribute("lede")){let i=a.note.split("\u2014"),l=r(i[0].trim()),d=r(i.slice(1).join("\u2014").trim());t+=`<p class="mk-fit-note"><strong>${l}</strong>${d?" \u2014 "+d:""}</p>`}let o=new Set(x),s=[];for(let i of a.languages)!o.has(i.band)&&!s.includes(i.band)&&s.push(i.band);for(let i of[...x,...s]){let l=a.languages.filter(d=>d.band===i);l.length!==0&&(t+='<section class="mk-fit-band"><div class="mk-fit-bandhead">',t+=`<span class="mk-fit-bandname">${H(i)} ${r(i)} fit</span>`,t+=`<span class="mk-fit-bandcount">${l.length} language${l.length>1?"s":""}</span>`,t+="</div>",n[i]&&(t+=`<p class="mk-fit-why">${r(n[i])}</p>`),t+=`<div class="mk-fit-grid">${l.map($).join("")}</div>`,t+="</section>")}t+="</div>";let c=this.getAttribute("footnote");c&&(t+=`<p class="mk-grid-foot">${p(c)}</p>`),e.innerHTML=t}});
