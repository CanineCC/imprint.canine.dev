...
`;function f(e){let a=e.getAttribute("kicker"),n=e.getAttribute("heading"),t=e.getAttribute("lede");if(!a&&!n&&!t)return"";let i='<div class="mk-section-head">';return a&&(i+=`<span class="mk-kicker">${s(a)}</span>`),n&&(i+=`<h2>${l(n)}</h2>`),t&&(i+=`<p>${l(t)}</p>`),i+="</div>",i}var m=`
...
`,d=class extends HTMLElement{#e;connectedCallback(){this.shadowRoot||this.attachShadow({mode:"open"}),this.#t(),this.render(this.shadowRoot),typeof this.liveLoad=="function"&&Promise.resolve(this.liveLoad()).catch(()=>{}),this.#e=new MutationObserver(()=>{let a=this.dataset.theme;this.#t(),this.dataset.theme!==a&&this.render(this.shadowRoot)}),this.#e.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}apiBase(){return(this.getAttribute("api-base")||"").trim()}disconnectedCallback(){this.#e?.disconnect()}#t(){let a=document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.dataset.theme=a;let n=(this.getAttribute("brand")||"").trim().toLowerCase();n==="assay"||n==="cai"||n==="watchdog"?this.dataset.brand=n:delete this.dataset.brand}json(a,n){let t=this.getAttribute(a);if(t==null||t.trim()==="")return n;try{return JSON.parse(t)}catch{return n}}};var g=new Map;function b(e,a,n){let t=(e||"").trim().replace(/\/$/,"");if(!t)return Promise.resolve(n);let i=t+" "+a,r=g.get(i);return r||(r=(async()=>{try{let o=await fetch(t+a);return o.ok?await o.json():n}catch{return n}})(),g.set(i,r),r)}async function h(e){let a=await b(e,"/api/public/language-support",{languages:[],bands:[]});return a&&Array.isArray(a.languages)?a:{languages:[],bands:[]}}var v={note:"FIT is survey clarity \u2014 how clearly a language leads to a complete architecture survey. It is NOT a measure of language or code quality.",languages:[{code:"csharp",displayName:"C#",applicability:10,supportKind:"Deep",band:"FULL",summary:"Native symbols resolve every lens with full fidelity.",coveredLenses:["DDD","Event sourcing","Event-driven","Vertical slice"],notApplicableLenses:[]},{code:"java",displayName:"Java",applicability:9,supportKind:"Deep",band:"FULL",summary:"Source-resolution is strong; the full domain survey fires.",coveredLenses:["DDD","Event sourcing","Event-driven","Vertical slice"],notApplicableLenses:["strongly-typed-id (field-based)"]},{code:"python",displayName:"Python",applicability:7,supportKind:"Deep",band:"HIGH",summary:"Marker lenses are strong; call-owner lenses partial on untyped code \u2014 declined, never guessed.",coveredLenses:["DDD","Event sourcing","Event-driven"],notApplicableLenses:["Sealed/DU lens"]},{code:"ruby",displayName:"Ruby",applicability:6,supportKind:"Deep",band:"MEDIUM",summary:"The hardest static target; markers and block-fold DSLs still resolve.",coveredLenses:["DDD","Event sourcing","Event-driven"],notApplicableLenses:["god-class/LCOM"]},{code:"javascript",displayName:"JavaScript",applicability:4,supportKind:"Structural",band:"LOW",summary:"Structural suite only, without types \u2014 the least architecture signal.",coveredLenses:["Structural","Module graph"],notApplicableLenses:["DDD","Event sourcing"]}],bands:[{band:"FULL",label:"FULL",why:"Every lens applies and resolves cleanly \u2014 the clearest surveys we produce. About survey clarity, not language quality."},{band:"HIGH",label:"HIGH",why:"Most lenses apply and resolve \u2014 a clear, well-populated survey. About survey clarity, not language quality."},{band:"MEDIUM",label:"MEDIUM",why:"A meaningful slice of the survey is populated; some lenses are N/A or best-effort. About survey clarity, not language quality."},{band:"LOW",label:"LOW",why:"The structural lenses apply but not the deep domain catalogue. What we report is real; there is simply less of it. About survey clarity, not language quality."}]},y=["FULL","HIGH","MEDIUM","LOW"],x={FULL:4,HIGH:3,MEDIUM:2,LOW:1},k=p+u+m+`
