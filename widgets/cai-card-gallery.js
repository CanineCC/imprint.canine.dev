...
`;function d(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function w(e){if(e==null||e==="")return"";let t=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,a="",r=0,n;for(;(n=t.exec(e))!==null;){n.index>r&&(a+=d(e.slice(r,n.index)));let i=n[0];if(i.startsWith("**"))a+=`<strong>${d(i.slice(2,-2))}</strong>`;else if(i.startsWith("`"))a+=`<code>${d(i.slice(1,-1))}</code>`;else{let s=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(i);s?a+=`<a href="${d(s[2])}">${d(s[1])}</a>`:a+=d(i)}r=n.index+i.length}return r<e.length&&(a+=d(e.slice(r))),a}var R=`
...
`;function z(e){let t=e.getAttribute("kicker"),a=e.getAttribute("heading"),r=e.getAttribute("lede");if(!t&&!a&&!r)return"";let n='<div class="mk-section-head">';return t&&(n+=`<span class="mk-kicker">${d(t)}</span>`),a&&(n+=`<h2>${w(a)}</h2>`),r&&(n+=`<p>${w(r)}</p>`),n+="</div>",n}var H=`
...
`,$=class extends HTMLElement{#e;connectedCallback(){this.shadowRoot||this.attachShadow({mode:"open"}),this.#a(),this.render(this.shadowRoot),typeof this.liveLoad=="function"&&Promise.resolve(this.liveLoad()).catch(()=>{}),this.#e=new MutationObserver(()=>{let t=this.dataset.theme;this.#a(),this.dataset.theme!==t&&this.render(this.shadowRoot)}),this.#e.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}apiBase(){return(this.getAttribute("api-base")||"").trim()}disconnectedCallback(){this.#e?.disconnect()}#a(){let t=document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.dataset.theme=t;let a=(this.getAttribute("brand")||"").trim().toLowerCase();a==="assay"||a==="cai"||a==="watchdog"?this.dataset.brand=a:delete this.dataset.brand}json(t,a){let r=this.getAttribute(t);if(r==null||r.trim()==="")return a;try{return JSON.parse(r)}catch{return a}}};var p=[{label:"Critical",key:"critical",floor:0},{label:"Weak",key:"poor",floor:25},{label:"Adequate",key:"fair",floor:50},{label:"Strong",key:"healthy",floor:70},{label:"Exemplary",key:"exemplary",floor:90}];function k(e){return e>=90?p[4]:e>=70?p[3]:e>=50?p[2]:e>=25?p[1]:p[0]}function I(e){let t=Math.max(0,Math.min(100,e)),a=p.length-1;for(;a>0&&t<p[a].floor;)a--;let n=(a<p.length-1?p[a+1].floor:100)-p[a].floor,i=n>0?Math.max(0,Math.min(1,(t-p[a].floor)/n)):.5;return{leftPct:a*20+i*20,third:i<1/3?0:i<2/3?1:2,key:p[a].key}}var W=0;function X(){return"s"+(W++).toString(36)}function q(e,{variant:t="diamond",caps:a=!1,className:r}={}){let n=I(e),i=k(e),s=`${n.leftPct.toFixed(2)}%`,f=p.map(h=>`<i class="seg-${h.key}"></i>`).join(""),o;t==="diamond"?o=`<div class="cai-mk cai-diamond" style="left:${s};--dia:var(--band-${i.key})"><span class="cai-diamond-foot"></span></div>`:o=`<div class="cai-mk cai-pin" style="left:${s}"><span class="cai-pin-badge">${Math.round(e)}</span><span class="cai-pin-line"></span><span class="cai-pin-foot"></span></div>`;let c=a?'<div class="cai-caps"><span>Worst</span><span>Best</span></div>':"";return`<div class="${"cai-ladder"+(a?"":" compact")+(r?` ${r}`:"")}"><div class="cai-rail" role="img" aria-label="${Math.round(e)} of 100 on a fixed worst-to-best scale"><div class="cai-segs">${f}</div>${o}</div>${c}</div>`}function J(e){let t=e[0],a=e[e.length-1],r=Math.round(a)-Math.round(t);return`Trend: ${r>=1?`improving (up ${r})`:r<=-1?`declining (down ${-r})`:"steady"} over the last ${e.length} scans.`}function K(e){let t=X(),a=320,r=36,n=4,i=e.length,s=Math.min(...e),f=Math.max(...e),o=Math.max(0,s-3),c=Math.min(100,f+3),u=c-o||1,h=l=>l/(i-1)*(a-2*n)+n,G=l=>n+(1-(l-o)/u)*(r-2*n),b=e.map((l,m)=>[h(m),G(l)]),M=`M ${b[0][0].toFixed(1)} ${b[0][1].toFixed(1)}`;for(let l=0;l<i-1;l++){let m=b[Math.max(0,l-1)],y=b[l],x=b[l+1],F=b[Math.min(i-1,l+2)],N=[y[0]+(x[0]-m[0])/6,y[1]+(x[1]-m[1])/6],B=[x[0]-(F[0]-y[0])/6,x[1]-(F[1]-y[1])/6];M+=` C ${N[0].toFixed(1)} ${N[1].toFixed(1)}, ${B[0].toFixed(1)} ${B[1].toFixed(1)}, ${x[0].toFixed(1)} ${x[1].toFixed(1)}`}let g=[],v=k(o).key;g.push({at:0,key:v});for(let l of p)if(l.floor>o&&l.floor<c){let m=(l.floor-o)/u;g.push({at:m,key:v}),v=l.key,g.push({at:m,key:v})}g.push({at:1,key:v});let P=g.map(l=>`<stop offset="${(l.at*100).toFixed(1)}%" stop-color="var(--band-${l.key})"></stop>`).join("");return`<svg class="cai-spark" viewBox="0 0 ${a} ${r}" preserveAspectRatio="none" aria-hidden="true" focusable="false"><defs><linearGradient id="caisg-${t}" gradientUnits="userSpaceOnUse" x1="0" y1="${r-n}" x2="0" y2="${n}">${P}</linearGradient></defs><path d="${M}" fill="none" stroke="url(#caisg-${t})" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"></path></svg><span class="sr-only">${d(J(e))}</span>`}function T(e){let t=Math.max(0,Math.min(100,Number(e.score)||0)),a=k(t),r=Array.isArray(e.series)&&e.series.length>=2?e.series:null,n=e.arcFirst!=null&&e.arcBest!=null&&Number.isFinite(Number(e.arcFirst))&&Number.isFinite(Number(e.arcBest)),i=n?Math.round(Number(e.arcBest))-Math.round(Number(e.arcFirst)):0,s=(e.lenses||[]).filter(c=>c&&c.label),f=(e.rows||[]).filter(c=>c&&c.label),o="";if(e.sealText&&(o+=`<span class="cai-seal">${d(e.sealText)}</span>`),o+=`<div class="cai-top"><span class="cai-name"><span class="cai-repo">${d(e.name)}</span>`+(e.owner?`<span class="cai-owner">by ${d(e.owner)}</span>`:"")+`</span><span class="cai-chip band-${a.key}">${d(a.label)}</span></div>`,o+=`<div class="cai-scoreline"><span class="cai-cai">CAI</span><span class="cai-score ink-${a.key}">${Math.round(t)}</span><span class="cai-unit"> / 100</span></div>`,o+=q(t,{variant:"diamond"}),r&&(o+=K(r)),n&&(o+=`<div class="cai-arc"><span class="cai-arc-from">${Math.round(Number(e.arcFirst))}</span><span class="cai-arc-arrow" aria-hidden="true">\u2192</span><span class="cai-arc-to ink-${a.key}">${Math.round(Number(e.arcBest))}</span>`+(i>=1?`<span class="cai-arc-up">\u2191 +${i}</span>`:"")+"</div>"),s.length>0){o+='<div class="cai-lenses">';for(let c of s){let u=c.value==null?null:Number(c.value),h=u==null?null:k(u);o+=`<div class="cai-lens"><span class="cai-lens-name">${d(c.label)}</span>`,o+='<span class="cai-lens-bar">',u!=null&&h&&(o+=`<span class="cai-lens-fill fill-${h.key}" style="width:${Math.max(2,Math.round(u))}%"></span>`),o+="</span>",u==null||!h?o+='<span class="cai-lens-num cai-muted">\u2014</span>':o+=`<span class="cai-lens-num ink-${h.key}">${Math.round(u)}</span>`,o+="</div>"}o+="</div>"}if(f.length>0){o+='<div class="cai-rows">';for(let c of f)o+=`<div class="cai-row"><span>${d(c.label)}</span><b class="${c.mono?"mono":""}">${d(c.value)}</b></div>`;o+="</div>"}return o}var _=`
...
`;function L(e){if(!e||!e.name||e.score==null)return null;let t;return Array.isArray(e.series)?t=e.series.map(a=>Number(a)).filter(a=>Number.isFinite(a)):t=String(e.series||"").split(",").map(a=>Number(a.trim())).filter(a=>Number.isFinite(a)),{name:e.name,owner:e.owner||void 0,score:Number(e.score),series:t.length>=2?t:void 0,arcFirst:e.arcFirst??null,arcBest:e.arcBest??null,lenses:(e.lenses||[]).filter(a=>a&&a.label).map(a=>({label:a.label,value:a.value??null})),rows:(e.rows||[]).filter(a=>a&&a.label).map(a=>({label:a.label,value:a.value||"",mono:!!a.mono})),href:e.href||void 0,sealText:e.sealText||void 0}}var Q=[["codeHealth","Code health"],["architecture","Architecture"],["maturity","Maturity"],["productionReadiness","Readiness"],["securityCompliance","Security"]];function V(e){return String(e).replace(/\B(?=(\d{3})+(?!\d))/g,",")}function Y(e){let t=e.publishedAt||e.lastUpdated,a="";if(t){let i=new Date(t);Number.isNaN(i.getTime())||(a=i.toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"}))}let r=e.productionLoc>0?`${V(e.productionLoc)} lines`:"",n=[a,r].filter(Boolean);return n.length?n.join(" \xB7 "):null}function O(e){if(!e)return null;let t=e.bestScore!=null?e.bestScore:e.headlineScore;if(t==null)return null;let a=Array.isArray(e.series)&&e.series.length>=2,r=Q.map(([s,f])=>({label:f,value:e[s]==null?null:Number(e[s])})).filter(s=>s.value!=null),n=[],i=Y(e);return i&&n.push({label:"Measured",value:i}),e.costApprox&&n.push({label:"Rebuild cost",value:e.costApprox}),e.busFactor>0&&e.authorCount>0&&n.push({label:"Bus factor",value:`${e.busFactor} of ${e.authorCount} devs`}),{name:e.name,owner:e.owner||void 0,score:Number(t),series:a?e.series.map(Number):void 0,arcFirst:a?e.firstScore:null,arcBest:a?e.bestScore:null,lenses:r,rows:n}}function S(e){if(!e)return!1;let t=e.sourceUrl||e.SourceUrl,a=e.bestRunId||e.BestRunId;return!!(t&&a)}function A(e,t){if(!S(e))return"";let a=(t||"").trim().replace(/\/$/,""),r=e.bestRunId||e.BestRunId;return a+"/api/oss/"+encodeURIComponent(e.owner)+"/"+encodeURIComponent(e.name)+"/report?run="+encodeURIComponent(r)}function U(e){return e.filter(S).slice().sort((t,a)=>{let r=t.bestScore!=null?t.bestScore:t.headlineScore;return(a.bestScore!=null?a.bestScore:a.headlineScore)-r||(a.productionLoc||0)-(t.productionLoc||0)})}function D(e,{exclude:t}={}){return e.filter(S).filter(a=>t?!(a.owner===t.owner&&a.name===t.name):!0).sort((a,r)=>{let n=a.bestScore!=null?a.bestScore:a.headlineScore;return(r.bestScore!=null?r.bestScore:r.headlineScore)-n||(r.productionLoc||0)-(a.productionLoc||0)})}async function j(e,t,a){if(!e)return a;try{let r=await fetch(e.replace(/\/$/,"")+t);return r.ok?await r.json():a}catch{return a}}async function C(e){if(!e)return!1;try{return(await fetch(e,{method:"GET"})).ok}catch{return!1}}var Z=6,ee=E+R+H+_+`
...
`;customElements.define("cai-card-gallery",class extends ${#e(){let e=this.getAttribute("count"),t=Number(e);return e!=null&&e!==""&&Number.isFinite(t)&&t>0?Math.floor(t):Z}async liveLoad(){let e=this.apiBase();if(!e)return;let t=await j(e,"/api/oss",null);if(!Array.isArray(t)||t.length===0)return;let a=U(t),r=null;for(let f of a)if(await C(A(f,e))){r=f;break}let n=r?{owner:r.owner,name:r.name}:void 0,i=this.#e(),s=[];for(let f of D(t,{exclude:n})){if(s.length>=i)break;let o=A(f,e);if(!o||!await C(o))continue;let c=O(f);c&&(c.href=o,s.push(c))}s.length!==0&&(this._live=s,this.render(this.shadowRoot))}render(e){let t=this._live?this._live:(this.json("cards",[])||[]).map(n=>L(n)).filter(Boolean),a=this.getAttribute("footnote"),r=`<style>${ee}</style>`;r+=z(this),r+='<div class="mk-cardgallery">';for(let n of t){let i=n.href?"a":"div",s=n.href?` href="${n.href.replace(/"/g,"&quot;")}"`:"";r+=`<${i} class="cai-card"${s}>${T(n)}</${i}>`}r+="</div>",a&&(r+=`<p class="mk-grid-foot">${w(a)}</p>`),e.innerHTML=r}});
