...
`;customElements.define("cai-card-gallery",class extends ${#e(){let e=this.getAttribute("count"),r=Number(e);return e!=null&&e!==""&&Number.isFinite(r)&&r>0?Math.floor(r):Z}async liveLoad(){let e=this.apiBase();if(!e)return;let r=await L(e);if(r.length===0)return;let a=j(r),t=U(r,a);if(t.length===0)return;let o=this.#e(),i=[];for(let s of t){if(i.length>=o)break;let p=I(s);if(!p)continue;let n=D(s,e);n&&(p.href=n),i.push(p)}i.length!==0&&(this._live=i,this.render(this.shadowRoot))}render(e){let r=this._live?this._live:(this.json("cards",[])||[]).map(o=>T(o)).filter(Boolean),a=this.getAttribute("footnote"),t=`<style>${ee}</style>`;t+=B(this),t+='<div class="mk-cardgallery">';for(let o of r){let i=o.href?"a":"div",s=o.href?` href="${o.href.replace(/"/g,"&quot;")}"`:"";t+=`<${i} class="cai-card"${s}>${E(o)}</${i}>`}t+="</div>",a&&(t+=`<p class="mk-grid-foot">${w(a)}</p>`),e.innerHTML=t}});
