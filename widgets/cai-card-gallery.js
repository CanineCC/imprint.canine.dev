...
`;function B(e){let r=e.getAttribute("kicker"),a=e.getAttribute("heading"),t=e.getAttribute("lede");if(!r&&!a&&!t)return"";let o='<div class="mk-section-head">';return r&&(o+=`<span class="mk-kicker">${p(r)}</span>`),a&&(o+=`<h2>${w(a)}</h2>`),t&&(o+=`<p>${w(t)}</p>`),o+="</div>",o}var H=`
...
`,$=class extends HTMLElement{#e;connectedCallback(){this.shadowRoot||this.attachShadow({mode:"open"}),this.#a(),this.render(this.shadowRoot),typeof this.liveLoad=="function"&&Promise.resolve(this.liveLoad()).catch(()=>{}),this.#e=new MutationObserver(()=>{let r=this.dataset.theme;this.#a(),this.dataset.theme!==r&&this.render(this.shadowRoot)}),this.#e.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}apiBase(){return(this.getAttribute("api-base")||"").trim()}disconnectedCallback(){this.#e?.disconnect()}#a(){let r=document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.dataset.theme=r;let a=(this.getAttribute("brand")||"").trim().toLowerCase();a==="assay"||a==="cai"||a==="watchdog"?this.dataset.brand=a:delete this.dataset.brand}json(r,a){let t=this.getAttribute(r);if(t==null||t.trim()==="")return a;try{return JSON.parse(t)}catch{return a}}};var f=[{label:"Critical",key:"critical",floor:0},{label:"Weak",key:"poor",floor:25},{label:"Adequate",key:"fair",floor:50},{label:"Strong",key:"healthy",floor:70},{label:"Exemplary",key:"exemplary",floor:90}];function y(e){return e>=90?f[4]:e>=70?f[3]:e>=50?f[2]:e>=25?f[1]:f[0]}function z(e){let r=Math.max(0,Math.min(100,e)),a=f.length-1;for(;a>0&&r<f[a].floor;)a--;let o=(a<f.length-1?f[a+1].floor:100)-f[a].floor,i=o>0?Math.max(0,Math.min(1,(r-f[a].floor)/o)):.5;return{leftPct:a*20+i*20,third:i<1/3?0:i<2/3?1:2,key:f[a].key}}var P=0;function W(){return"s"+(P++).toString(36)}function X(e,{variant:r="diamond",caps:a=!1,className:t}={}){let o=z(e),i=y(e),s=`${o.leftPct.toFixed(2)}%`,d=f.map(h=>`<i class="seg-${h.key}"></i>`).join(""),n;r==="diamond"?n=`<div class="cai-mk cai-diamond" style="left:${s};--dia:var(--band-${i.key})"><span class="cai-diamond-foot"></span></div>`:n=`<div class="cai-mk cai-pin" style="left:${s}"><span class="cai-pin-badge">${Math.round(e)}</span><span class="cai-pin-line"></span><span class="cai-pin-foot"></span></div>`;let c=a?'<div class="cai-caps"><span>Worst</span><span>Best</span></div>':"";return`<div class="${"cai-ladder"+(a?"":" compact")+(t?` ${t}`:"")}"><div class="cai-rail" role="img" aria-label="${Math.round(e)} of 100 on a fixed worst-to-best scale"><div class="cai-segs">${d}</div>${n}</div>${c}</div>`}function q(e){let r=e[0],a=e[e.length-1],t=Math.round(a)-Math.round(r);return`Trend: ${t>=1?`improving (up ${t})`:t<=-1?`declining (down ${-t})`:"steady"} over the last ${e.length} scans.`}function J(e){let r=W(),a=320,t=36,o=4,i=e.length,s=Math.min(...e),d=Math.max(...e),n=Math.max(0,s-3),c=Math.min(100,d+3),u=c-n||1,h=l=>l/(i-1)*(a-2*o)+o,O=l=>o+(1-(l-n)/u)*(t-2*o),b=e.map((l,m)=>[h(m),O(l)]),S=`M ${b[0][0].toFixed(1)} ${b[0][1].toFixed(1)}`;for(let l=0;l<i-1;l++){let m=b[Math.max(0,l-1)],k=b[l],g=b[l+1],A=b[Math.min(i-1,l+2)],C=[k[0]+(g[0]-m[0])/6,k[1]+(g[1]-m[1])/6],N=[g[0]-(A[0]-k[0])/6,g[1]-(A[1]-k[1])/6];S+=` C ${C[0].toFixed(1)} ${C[1].toFixed(1)}, ${N[0].toFixed(1)} ${N[1].toFixed(1)}, ${g[0].toFixed(1)} ${g[1].toFixed(1)}`}let x=[],v=y(n).key;x.push({at:0,key:v});for(let l of f)if(l.floor>n&&l.floor<c){let m=(l.floor-n)/u;x.push({at:m,key:v}),v=l.key,x.push({at:m,key:v})}x.push({at:1,key:v});let G=x.map(l=>`<stop offset="${(l.at*100).toFixed(1)}%" stop-color="var(--band-${l.key})"></stop>`).join("");return`<svg class="cai-spark" viewBox="0 0 ${a} ${t}" preserveAspectRatio="none" aria-hidden="true" focusable="false"><defs><linearGradient id="caisg-${r}" gradientUnits="userSpaceOnUse" x1="0" y1="${t-o}" x2="0" y2="${o}">${G}</linearGradient></defs><path d="${S}" fill="none" stroke="url(#caisg-${r})" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"></path></svg><span class="sr-only">${p(q(e))}</span>`}function E(e){let r=Math.max(0,Math.min(100,Number(e.score)||0)),a=y(r),t=Array.isArray(e.series)&&e.series.length>=2?e.series:null,o=e.arcFirst!=null&&e.arcBest!=null&&Number.isFinite(Number(e.arcFirst))&&Number.isFinite(Number(e.arcBest)),i=o?Math.round(Number(e.arcBest))-Math.round(Number(e.arcFirst)):0,s=(e.lenses||[]).filter(c=>c&&c.label),d=(e.rows||[]).filter(c=>c&&c.label),n="";if(e.sealText&&(n+=`<span class="cai-seal">${p(e.sealText)}</span>`),n+=`<div class="cai-top"><span class="cai-name"><span class="cai-repo">${p(e.name)}</span>`+(e.owner?`<span class="cai-owner">by ${p(e.owner)}</span>`:"")+`</span><span class="cai-chip band-${a.key}">${p(a.label)}</span></div>`,n+=`<div class="cai-scoreline"><span class="cai-cai">CAI</span><span class="cai-score ink-${a.key}">${Math.round(r)}</span><span class="cai-unit"> / 100</span></div>`,n+=X(r,{variant:"diamond"}),t&&(n+=J(t)),o&&(n+=`<div class="cai-arc"><span class="cai-arc-from">${Math.round(Number(e.arcFirst))}</span><span class="cai-arc-arrow" aria-hidden="true">\u2192</span><span class="cai-arc-to ink-${a.key}">${Math.round(Number(e.arcBest))}</span>`+(i>=1?`<span class="cai-arc-up">\u2191 +${i}</span>`:"")+"</div>"),s.length>0){n+='<div class="cai-lenses">';for(let c of s){let u=c.value==null?null:Number(c.value),h=u==null?null:y(u);n+=`<div class="cai-lens"><span class="cai-lens-name">${p(c.label)}</span>`,n+='<span class="cai-lens-bar">',u!=null&&h&&(n+=`<span class="cai-lens-fill fill-${h.key}" style="width:${Math.max(2,Math.round(u))}%"></span>`),n+="</span>",u==null||!h?n+='<span class="cai-lens-num cai-muted">\u2014</span>':n+=`<span class="cai-lens-num ink-${h.key}">${Math.round(u)}</span>`,n+="</div>"}n+="</div>"}if(d.length>0){n+='<div class="cai-rows">';for(let c of d)n+=`<div class="cai-row"><span>${p(c.label)}</span><b class="${c.mono?"mono":""}">${p(c.value)}</b></div>`;n+="</div>"}return n}var R=`
