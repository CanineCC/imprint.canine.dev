...
`;function p(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function w(e){if(e==null||e==="")return"";let r=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,a="",t=0,o;for(;(o=r.exec(e))!==null;){o.index>t&&(a+=p(e.slice(t,o.index)));let i=o[0];if(i.startsWith("**"))a+=`<strong>${p(i.slice(2,-2))}</strong>`;else if(i.startsWith("`"))a+=`<code>${p(i.slice(1,-1))}</code>`;else{let s=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(i);s?a+=`<a href="${p(s[2])}">${p(s[1])}</a>`:a+=p(i)}t=o.index+i.length}return t<e.length&&(a+=p(e.slice(t))),a}var H=`
...
`;function F(e){let r=e.getAttribute("kicker"),a=e.getAttribute("heading"),t=e.getAttribute("lede");if(!r&&!a&&!t)return"";let o='<div class="mk-section-head">';return r&&(o+=`<span class="mk-kicker">${p(r)}</span>`),a&&(o+=`<h2>${w(a)}</h2>`),t&&(o+=`<p>${w(t)}</p>`),o+="</div>",o}var E=`
...
`,$=class extends HTMLElement{#e;connectedCallback(){this.shadowRoot||this.attachShadow({mode:"open"}),this.#a(),this.render(this.shadowRoot),typeof this.liveLoad=="function"&&Promise.resolve(this.liveLoad()).catch(()=>{}),this.#e=new MutationObserver(()=>{let r=this.dataset.theme;this.#a(),this.dataset.theme!==r&&this.render(this.shadowRoot)}),this.#e.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}apiBase(){return(this.getAttribute("api-base")||"").trim()}disconnectedCallback(){this.#e?.disconnect()}#a(){let r=document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.dataset.theme=r;let a=(this.getAttribute("brand")||"").trim().toLowerCase();a==="assay"||a==="cai"||a==="watchdog"?this.dataset.brand=a:delete this.dataset.brand}json(r,a){let t=this.getAttribute(r);if(t==null||t.trim()==="")return a;try{return JSON.parse(t)}catch{return a}}};var u=[{label:"Critical",key:"critical",floor:0},{label:"Weak",key:"poor",floor:25},{label:"Adequate",key:"fair",floor:50},{label:"Strong",key:"healthy",floor:70},{label:"Exemplary",key:"exemplary",floor:90}];function y(e){return e>=90?u[4]:e>=70?u[3]:e>=50?u[2]:e>=25?u[1]:u[0]}function L(e){let r=Math.max(0,Math.min(100,e)),a=u.length-1;for(;a>0&&r<u[a].floor;)a--;let o=(a<u.length-1?u[a+1].floor:100)-u[a].floor,i=o>0?Math.max(0,Math.min(1,(r-u[a].floor)/o)):.5;return{leftPct:a*20+i*20,third:i<1/3?0:i<2/3?1:2,key:u[a].key}}var O=0;function q(){return"s"+(O++).toString(36)}function W(e,{variant:r="diamond",caps:a=!1,className:t}={}){let o=L(e),i=y(e),s=`${o.leftPct.toFixed(2)}%`,d=u.map(h=>`<i class="seg-${h.key}"></i>`).join(""),n;r==="diamond"?n=`<div class="cai-mk cai-diamond" style="left:${s};--dia:var(--band-${i.key})"><span class="cai-diamond-foot"></span></div>`:n=`<div class="cai-mk cai-pin" style="left:${s}"><span class="cai-pin-badge">${Math.round(e)}</span><span class="cai-pin-line"></span><span class="cai-pin-foot"></span></div>`;let c=a?'<div class="cai-caps"><span>Worst</span><span>Best</span></div>':"";return`<div class="${"cai-ladder"+(a?"":" compact")+(t?` ${t}`:"")}"><div class="cai-rail" role="img" aria-label="${Math.round(e)} of 100 on a fixed worst-to-best scale"><div class="cai-segs">${d}</div>${n}</div>${c}</div>`}function J(e){let r=e[0],a=e[e.length-1],t=Math.round(a)-Math.round(r);return`Trend: ${t>=1?`improving (up ${t})`:t<=-1?`declining (down ${-t})`:"steady"} over the last ${e.length} scans.`}function X(e){let r=q(),a=320,t=36,o=4,i=e.length,s=Math.min(...e),d=Math.max(...e),n=Math.max(0,s-3),c=Math.min(100,d+3),f=c-n||1,h=l=>l/(i-1)*(a-2*o)+o,D=l=>o+(1-(l-n)/f)*(t-2*o),g=e.map((l,m)=>[h(m),D(l)]),S=`M ${g[0][0].toFixed(1)} ${g[0][1].toFixed(1)}`;for(let l=0;l<i-1;l++){let m=g[Math.max(0,l-1)],k=g[l],b=g[l+1],A=g[Math.min(i-1,l+2)],C=[k[0]+(b[0]-m[0])/6,k[1]+(b[1]-m[1])/6],N=[b[0]-(A[0]-k[0])/6,b[1]-(A[1]-k[1])/6];S+=` C ${C[0].toFixed(1)} ${C[1].toFixed(1)}, ${N[0].toFixed(1)} ${N[1].toFixed(1)}, ${b[0].toFixed(1)} ${b[1].toFixed(1)}`}let x=[],v=y(n).key;x.push({at:0,key:v});for(let l of u)if(l.floor>n&&l.floor<c){let m=(l.floor-n)/f;x.push({at:m,key:v}),v=l.key,x.push({at:m,key:v})}x.push({at:1,key:v});let G=x.map(l=>`<stop offset="${(l.at*100).toFixed(1)}%" stop-color="var(--band-${l.key})"></stop>`).join("");return`<svg class="cai-spark" viewBox="0 0 ${a} ${t}" preserveAspectRatio="none" aria-hidden="true" focusable="false"><defs><linearGradient id="caisg-${r}" gradientUnits="userSpaceOnUse" x1="0" y1="${t-o}" x2="0" y2="${o}">${G}</linearGradient></defs><path d="${S}" fill="none" stroke="url(#caisg-${r})" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"></path></svg><span class="sr-only">${p(J(e))}</span>`}function z(e){let r=Math.max(0,Math.min(100,Number(e.score)||0)),a=y(r),t=Array.isArray(e.series)&&e.series.length>=2?e.series:null,o=e.arcFirst!=null&&e.arcBest!=null&&Number.isFinite(Number(e.arcFirst))&&Number.isFinite(Number(e.arcBest)),i=o?Math.round(Number(e.arcBest))-Math.round(Number(e.arcFirst)):0,s=(e.lenses||[]).filter(c=>c&&c.label),d=(e.rows||[]).filter(c=>c&&c.label),n="";if(e.sealText&&(n+=`<span class="cai-seal">${p(e.sealText)}</span>`),n+=`<div class="cai-top"><span class="cai-name"><span class="cai-repo">${p(e.name)}</span>`+(e.owner?`<span class="cai-owner">by ${p(e.owner)}</span>`:"")+`</span><span class="cai-chip band-${a.key}">${p(a.label)}</span></div>`,n+=`<div class="cai-scoreline"><span class="cai-cai">CAI</span><span class="cai-score ink-${a.key}">${Math.round(r)}</span><span class="cai-unit"> / 100</span></div>`,n+=W(r,{variant:"diamond"}),t&&(n+=X(t)),o&&(n+=`<div class="cai-arc"><span class="cai-arc-from">${Math.round(Number(e.arcFirst))}</span><span class="cai-arc-arrow" aria-hidden="true">\u2192</span><span class="cai-arc-to ink-${a.key}">${Math.round(Number(e.arcBest))}</span>`+(i>=1?`<span class="cai-arc-up">\u2191 +${i}</span>`:"")+"</div>"),s.length>0){n+='<div class="cai-lenses">';for(let c of s){let f=c.value==null?null:Number(c.value),h=f==null?null:y(f);n+=`<div class="cai-lens"><span class="cai-lens-name">${p(c.label)}</span>`,n+='<span class="cai-lens-bar">',f!=null&&h&&(n+=`<span class="cai-lens-fill fill-${h.key}" style="width:${Math.max(2,Math.round(f))}%"></span>`),n+="</span>",f==null||!h?n+='<span class="cai-lens-num cai-muted">\u2014</span>':n+=`<span class="cai-lens-num ink-${h.key}">${Math.round(f)}</span>`,n+="</div>"}n+="</div>"}if(d.length>0){n+='<div class="cai-rows">';for(let c of d)n+=`<div class="cai-row"><span>${p(c.label)}</span><b class="${c.mono?"mono":""}">${p(c.value)}</b></div>`;n+="</div>"}return n}var R=`
...
`;function B(e){if(!e||!e.name||e.score==null)return null;let r;return Array.isArray(e.series)?r=e.series.map(a=>Number(a)).filter(a=>Number.isFinite(a)):r=String(e.series||"").split(",").map(a=>Number(a.trim())).filter(a=>Number.isFinite(a)),{name:e.name,owner:e.owner||void 0,score:Number(e.score),series:r.length>=2?r:void 0,arcFirst:e.arcFirst??null,arcBest:e.arcBest??null,lenses:(e.lenses||[]).filter(a=>a&&a.label).map(a=>({label:a.label,value:a.value??null})),rows:(e.rows||[]).filter(a=>a&&a.label).map(a=>({label:a.label,value:a.value||"",mono:!!a.mono})),href:e.href||void 0,sealText:e.sealText||void 0}}var K=[["codeHealth","Code health"],["architecture","Architecture"],["maturity","Maturity"],["productionReadiness","Readiness"],["securityCompliance","Security"]];function Q(e){return String(e).replace(/\B(?=(\d{3})+(?!\d))/g,",")}function V(e){let r=e.publishedAt||e.lastUpdated,a="";if(r){let i=new Date(r);Number.isNaN(i.getTime())||(a=i.toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"}))}let t=e.productionLoc>0?`${Q(e.productionLoc)} lines`:"",o=[a,t].filter(Boolean);return o.length?o.join(" \xB7 "):null}function U(e){if(!e)return null;let r=e.bestScore!=null?e.bestScore:e.headlineScore;if(r==null)return null;let a=Array.isArray(e.series)&&e.series.length>=2,t=K.map(([d,n])=>({label:n,value:e[d]==null?null:Number(e[d])})).filter(d=>d.value!=null),o=[],i=V(e);i&&o.push({label:"Measured",value:i}),e.costApprox&&o.push({label:"Rebuild cost",value:e.costApprox}),e.busFactor>0&&e.authorCount>0&&o.push({label:"Bus factor",value:`${e.busFactor} of ${e.authorCount} devs`});let s=typeof e.display=="string"&&e.display.length>0&&e.display!==(e.owner?e.owner+"/"+e.name:e.name);return{name:s?e.display:e.name,owner:s?void 0:e.owner||void 0,score:Number(r),series:a?e.series.map(Number):void 0,arcFirst:a?e.firstScore:null,arcBest:a?e.bestScore:null,lenses:t,rows:o}}function P(e,r){if(!e)return"";let a=e.bestRunId||e.BestRunId,t=e.owner||e.Owner,o=e.name||e.Name;return!a||!t||!o?"":(r||"").trim().replace(/\/$/,"")+"/api/oss/"+encodeURIComponent(t)+"/"+encodeURIComponent(o)+"/report?run="+encodeURIComponent(a)}var T=new Map;function Y(e,r,a){let t=(e||"").trim().replace(/\/$/,"");if(!t)return Promise.resolve(a);let o=t+" "+r,i=T.get(o);return i||(i=(async()=>{try{let s=await fetch(t+r);return s.ok?await s.json():a}catch{return a}})(),T.set(o,i),i)}async function _(e){let r=await Y(e,"/api/oss",[]);return Array.isArray(r)?r:[]}function j(e){if(!Array.isArray(e)||e.length===0)return null;let r=t=>t.bestScore!=null?Number(t.bestScore):Number(t.headlineScore)||0,a=null;for(let t of e)(a===null||r(t)>r(a)||r(t)===r(a)&&(Number(t.productionLoc)||0)>(Number(a.productionLoc)||0))&&(a=t);return a}function I(e,r){let a=(Array.isArray(e)?e:[]).filter(n=>n&&n!==r);if(a.length===0)return[];let t=n=>n.bestScore!=null?Number(n.bestScore):Number(n.headlineScore)||0,o=n=>n.delta!=null?Number(n.delta):0,i=[],s=n=>{n&&!i.includes(n)&&i.push(n)};s([...a].sort((n,c)=>t(c)-t(n))[0]),s([...a].sort((n,c)=>o(c)-o(n)).find(n=>!i.includes(n)));let d=a.filter(n=>!i.includes(n));return d.length>0&&s(d[Math.floor(Math.random()*d.length)]),i}var Z=6,ee=M+H+E+R+`
...
`;customElements.define("cai-card-gallery",class extends ${#e(){let e=this.getAttribute("count"),r=Number(e);return e!=null&&e!==""&&Number.isFinite(r)&&r>0?Math.floor(r):Z}async liveLoad(){let e=this.apiBase();if(!e)return;let r=await _(e);if(r.length===0)return;let a=j(r),t=I(r,a);if(t.length===0)return;let o=this.#e(),i=[];for(let s of t){if(i.length>=o)break;let d=U(s);if(!d)continue;let n=P(s,e);n&&(d.href=n),i.push(d)}i.length!==0&&(this._live=i,this.render(this.shadowRoot))}render(e){let r=this._live?this._live:(this.json("cards",[])||[]).map(o=>B(o)).filter(Boolean),a=this.getAttribute("footnote"),t=`<style>${ee}</style>`;t+=F(this),t+='<div class="mk-cardgallery">';for(let o of r){let i=o.href?"a":"div",s=o.href?` href="${o.href.replace(/"/g,"&quot;")}"`:"";t+=`<${i} class="cai-card"${s}>${z(o)}</${i}>`}t+="</div>",a&&(t+=`<p class="mk-grid-foot">${w(a)}</p>`),e.innerHTML=t}});
