...
`;function s(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function f(t){if(t==null||t==="")return"";let a=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,e="",r=0,n;for(;(n=a.exec(t))!==null;){n.index>r&&(e+=s(t.slice(r,n.index)));let o=n[0];if(o.startsWith("**"))e+=`<strong>${s(o.slice(2,-2))}</strong>`;else if(o.startsWith("`"))e+=`<code>${s(o.slice(1,-1))}</code>`;else{let c=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(o);c?e+=`<a href="${s(c[2])}">${s(c[1])}</a>`:e+=s(o)}r=n.index+o.length}return r<t.length&&(e+=s(t.slice(r))),e}var m=`
...
`;function b(t){let a=t.getAttribute("kicker"),e=t.getAttribute("heading"),r=t.getAttribute("lede");if(!a&&!e&&!r)return"";let n='<div class="mk-section-head">';return a&&(n+=`<span class="mk-kicker">${s(a)}</span>`),e&&(n+=`<h2>${f(e)}</h2>`),r&&(n+=`<p>${f(r)}</p>`),n+="</div>",n}var y=`
...
`,h=class extends HTMLElement{#e;connectedCallback(){this.shadowRoot||this.attachShadow({mode:"open"}),this.#t(),this.render(this.shadowRoot),typeof this.liveLoad=="function"&&Promise.resolve(this.liveLoad()).catch(()=>{}),this.#e=new MutationObserver(()=>{let a=this.dataset.theme;this.#t(),this.dataset.theme!==a&&this.render(this.shadowRoot)}),this.#e.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}apiBase(){return(this.getAttribute("api-base")||"").trim()}disconnectedCallback(){this.#e?.disconnect()}#t(){let a=document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.dataset.theme=a;let e=(this.getAttribute("brand")||"").trim().toLowerCase();e==="assay"||e==="cai"||e==="watchdog"?this.dataset.brand=e:delete this.dataset.brand}json(a,e){let r=this.getAttribute(a);if(r==null||r.trim()==="")return e;try{return JSON.parse(r)}catch{return e}}};var x=new Map;function N(t,a,e){let r=(t||"").trim().replace(/\/$/,"");if(!r)return Promise.resolve(e);let n=r+" "+a,o=x.get(n);return o||(o=(async()=>{try{let c=await fetch(r+a);return c.ok?await c.json():e}catch{return e}})(),x.set(n,o),o)}async function v(t){let a=await N(t,"/api/oss",[]);return Array.isArray(a)?a:[]}function k(t){if(!Array.isArray(t)||t.length===0)return null;let a=r=>r.bestScore!=null?Number(r.bestScore):Number(r.headlineScore)||0,e=null;for(let r of t)(e===null||a(r)>a(e)||a(r)===a(e)&&(Number(r.productionLoc)||0)>(Number(e.productionLoc)||0))&&(e=r);return e}var L={exemplary:"var(--band-exemplary)",healthy:"var(--band-healthy)",fair:"var(--band-fair)",poor:"var(--band-poor)",critical:"var(--band-critical)"},H={exemplary:.85,healthy:.85,fair:.7,poor:.8,critical:.8},E=g+m+y+`
...
`;function P(t){if(!t)return null;let a=t.brilliantPct==null?null:Number(t.brilliantPct),e=t.slopPct==null?null:Number(t.slopPct);if(a==null||e==null)return null;let r=t.finePct!=null?Math.max(0,Number(t.finePct)):Math.max(0,100-a-e),n=o=>Math.round(o*10)/10;return[{label:"Brilliant",pct:n(a),band:"exemplary"},{label:"Fine",pct:n(r),band:"fair"},{label:"Slop",pct:n(e),band:"critical"}].filter(o=>o.pct>0)}customElements.define("cai-composition-bar",class extends h{async liveLoad(){let t=this.apiBase();if(!t)return;let a=P(k(await v(t)));!a||a.length===0||(this._live=a,this.render(this.shadowRoot))}render(t){let a=(this._live||this.json("segments",[])||[]).filter(i=>i&&Number(i.pct)>0),e=a.reduce((i,l)=>i+(Number(l.pct)||0),0)||100,r=760,n=72,o=this.getAttribute("caption"),c=0,w=a.map(i=>{let l=Number(i.pct)/e*r,p={seg:i,x:c,w:l};return c+=l,p}),S="Code composition: "+a.map(i=>`${i.pct}% ${String(i.label||"").toLowerCase()}`).join(", "),d=`<svg viewBox="0 0 ${r} 116" role="img" aria-label="${s(S)}">`;for(let{seg:i,x:l,w:p}of w){let A=L[i.band]||"var(--band-fair)",C=H[i.band]??.8,$=i.band==="fair"?500:700;d+="<g>",d+=`<rect x="${l}" y="0" width="${p}" height="${n}" fill="${A}" opacity="${C}"></rect>`,d+=`<text class="mk-compbar-label" x="${l+p/2}" y="${n/2+5}" text-anchor="middle" font-weight="${$}">${s(i.label)}</text>`,d+=`<text class="mk-compbar-pct" x="${l+2}" y="${n+30}">${s(String(i.pct))}%</text>`,d+="</g>"}d+="</svg>";let u=`<style>${E}</style>`;u+=b(this),u+=`<figure class="mk-compbar">${d}`,o&&(u+=`<figcaption class="mk-compbar-cap">${f(o)}</figcaption>`),u+="</figure>",t.innerHTML=u}});
