...
customElements.define(
  ...
  class extends CaiIsland {
    ...
    render(root) {
      ...
      const coded = dims.filter((d) => (d.family || "dimension") === "dimension");
      ...
      if (lenses.length) {
        ...
        for (const l of lenses) {
          ...
          const n = coded.filter((d) => d.lens === key).length;
          ...
          h += `<div class="dx-lens-count">${n} coded dimension${n === 1 ? "" : "s"}</div>`;
          h += "</div>";
        }
        h += "</div>";
      }

      // The D-family, in code order.
      const ordered = [...coded].sort((a, b) => {
      ...
      h += "<thead><tr><th>Code</th><th>Dimension</th><th>What it measures</th><th>Lens</th><th>Evaluator</th></tr></thead><tbody>";
