...
`;function c(a){return String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function u(a){if(a==null||a==="")return"";let i=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,e="",s=0,r;for(;(r=i.exec(a))!==null;){r.index>s&&(e+=c(a.slice(s,r.index)));let l=r[0];if(l.startsWith("**"))e+=`<strong>${c(l.slice(2,-2))}</strong>`;else if(l.startsWith("`"))e+=`<code>${c(l.slice(1,-1))}</code>`;else{let t=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(l);t?e+=`<a href="${c(t[2])}">${c(t[1])}</a>`:e+=c(l)}s=r.index+l.length}return s<a.length&&(e+=c(a.slice(s))),e}var g=`
...
`;function v(a){let i=a.getAttribute("kicker"),e=a.getAttribute("heading"),s=a.getAttribute("lede");if(!i&&!e&&!s)return"";let r='<div class="mk-section-head">';return i&&(r+=`<span class="mk-kicker">${c(i)}</span>`),e&&(r+=`<h2>${u(e)}</h2>`),s&&(r+=`<p>${u(s)}</p>`),r+="</div>",r}var k=`
...
`,w=[["score-desc","Highest score first"],["score-asc","Lowest score first"],["name","Name A\u2013Z"],["size-desc","Largest first"]],S=["recent","Most recently measured"];customElements.define("cai-survey-list",class extends h{render(a){let i=(this.json("projects",[])||[]).filter(t=>t&&t.name);this._rows=i,this._q=this._q||"",this._sort=this._sort||"score-desc",this._bands=this._bands||null;let e=`<style>${$}</style>`;if(e+=v(this),i.length===0){a.innerHTML=e;return}let s=new Map(p.map(t=>[t.key,0]));for(let t of i){let o=f(Number(t.score)).key;s.set(o,(s.get(o)||0)+1)}let r=p.filter(t=>s.get(t.key)>0);e+='<div class="mk-list">',e+='<div class="mk-dist" role="group" aria-label="Filter by band">';for(let t of r){let o=s.get(t.key),n=!this._bands||this._bands.has(t.key);e+=`<button type="button" class="fill-${t.key}" data-band="${t.key}" style="flex: ${o} 0 0" aria-pressed="${n}" title="${c(`${t.label}: ${o} project${o===1?"":"s"}`)}">${o}</button>`}e+='</div><p class="mk-dist-key">';for(let t of r)e+=`<span><i class="fill-${t.key}"></i>${c(t.label)} ${s.get(t.key)}</span>`;e+="</p>",e+='<div class="mk-ctl">',e+=`<input type="search" class="mk-q" placeholder="Search these projects" aria-label="Search these projects" value="${c(this._q)}">`;let l=i.some(t=>t.at)?[...w,S]:w;e+='<select class="mk-sort" aria-label="Sort">';for(let[t,o]of l)e+=`<option value="${t}"${t===this._sort?" selected":""}>${c(o)}</option>`;e+="</select>",(this._q||this._bands)&&(e+='<button type="button" class="mk-ctl-clear">Clear</button>'),e+="</div>",e+='<p class="mk-count" role="status"></p>',e+='<div class="mk-cards"></div>',e+="</div>",a.innerHTML=e,this.wire(a),this.paint(a)}wire(a){let i=a.querySelector(".mk-q"),e=a.querySelector(".mk-sort");i?.addEventListener("input",()=>{this._q=i.value,this.paint(a)}),e?.addEventListener("change",()=>{this._sort=e.value,this.paint(a)}),a.querySelector(".mk-ctl-clear")?.addEventListener("click",()=>{this._q="",this._bands=null,this.render(a)});for(let s of a.querySelectorAll(".mk-dist button"))s.addEventListener("click",()=>{let r=s.getAttribute("data-band"),l=this._bands?new Set(this._bands):new Set;this._bands&&l.has(r)?l.delete(r):l.add(r),this._bands=l.size===0?null:l,this.render(a)})}paint(a){let i=a.querySelector(".mk-cards"),e=a.querySelector(".mk-count");if(!i||!e)return;let s=this._q.trim().toLowerCase(),r=this._rows.filter(n=>this._bands&&!this._bands.has(f(Number(n.score)).key)?!1:s?`${n.owner||""}/${n.name}`.toLowerCase().includes(s):!0),l={"score-desc":(n,d)=>Number(d.score)-Number(n.score),"score-asc":(n,d)=>Number(n.score)-Number(d.score),name:(n,d)=>String(n.name).localeCompare(String(d.name)),"size-desc":(n,d)=>Number(d.loc||0)-Number(n.loc||0),recent:(n,d)=>String(d.at||"").localeCompare(String(n.at||""))}[this._sort];r=[...r].sort(l||(()=>0));let t=this._rows.length;if(e.textContent=r.length===t?`${t} project${t===1?"":"s"}`:`${r.length} of ${t} projects`,r.length===0){i.innerHTML=`<p class="mk-empty">${c(this.getAttribute("empty-text")||"No project here matches that.")}</p>`;return}let o="";for(let n of r){let d=Number(n.score),b=f(d),m=[];n.loc&&m.push(`${_(Number(n.loc))} lines`),Number(n.busFactor)===1&&m.push("bus factor 1"),n.day&&m.push(`measured ${n.day}`),o+=`<a class="mk-card" href="${c(n.href||"#")}">`,o+='<span class="mk-card-top">',o+='<span class="mk-card-name">',n.owner&&(o+=`<span class="mk-card-owner">${c(n.owner)}/</span>`),o+=`${c(n.name)}</span>`,o+=`<span class="mk-card-score ink-${b.key}">${d.toFixed(1)}</span>`,o+="</span>",o+=`<span class="cai-lens-bar"><span class="cai-lens-fill fill-${b.key}" style="width:${Math.max(2,Math.round(d))}%"></span></span>`,o+=`<span class="mk-card-meta">${c([b.label,...m].join(" \xB7 "))}</span>`,o+="</a>"}i.innerHTML=o}});function _(a){return!Number.isFinite(a)||a<=0?"0":a>=1e6?`${(a/1e6).toFixed(1)}m`:a>=1e3?`${Math.round(a/1e3)}k`:String(a)}
