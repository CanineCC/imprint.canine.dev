...
`,w=[["score-desc","Highest score first"],["score-asc","Lowest score first"],["name","Name A\u2013Z"],["size-desc","Largest first"]],S=["recent","Most recently measured"];customElements.define("cai-survey-list",class extends h{render(a){let i=(this.json("projects",[])||[]).filter(t=>t&&t.name);this._rows=i,this._q=this._q||"",this._sort=this._sort||"score-desc",this._bands=this._bands||null;let e=`<style>${$}</style>`;if(e+=v(this),i.length===0){a.innerHTML=e;return}let s=new Map(p.map(t=>[t.key,0]));for(let t of i){let o=m(Number(t.score)).key;s.set(o,(s.get(o)||0)+1)}let n=p.filter(t=>s.get(t.key)>0);e+='<div class="mk-list">',e+='<div class="mk-dist" role="group" aria-label="Filter by band">';for(let t of n){let o=s.get(t.key),r=!this._bands||this._bands.has(t.key);e+=`<button type="button" class="fill-${t.key}" data-band="${t.key}" style="flex: ${o} 0 0" aria-pressed="${r}" title="${c(`${t.label}: ${o} project${o===1?"":"s"}`)}">${o}</button>`}e+='</div><p class="mk-dist-key">';for(let t of n)e+=`<span><i class="fill-${t.key}"></i>${c(t.label)} ${s.get(t.key)}</span>`;e+="</p>",e+='<div class="mk-ctl">',e+=`<input type="search" class="mk-q" placeholder="Search these projects" aria-label="Search these projects" value="${c(this._q)}">`;let l=i.some(t=>t.at)?[...w,S]:w;e+='<select class="mk-sort" aria-label="Sort">';for(let[t,o]of l)e+=`<option value="${t}"${t===this._sort?" selected":""}>${c(o)}</option>`;e+="</select>",(this._q||this._bands)&&(e+='<button type="button" class="mk-ctl-clear">Clear</button>'),e+="</div>",e+='<p class="mk-count" role="status"></p>',e+='<div class="mk-cards"></div>',e+="</div>",a.innerHTML=e,this.wire(a),this.paint(a)}wire(a){let i=a.querySelector(".mk-q"),e=a.querySelector(".mk-sort");i?.addEventListener("input",()=>{this._q=i.value,this.paint(a)}),e?.addEventListener("change",()=>{this._sort=e.value,this.paint(a)}),a.querySelector(".mk-ctl-clear")?.addEventListener("click",()=>{this._q="",this._bands=null,this.render(a)});for(let s of a.querySelectorAll(".mk-dist button"))s.addEventListener("click",()=>{let n=s.getAttribute("data-band"),l=this._bands?new Set(this._bands):new Set;this._bands&&l.has(n)?l.delete(n):l.add(n),this._bands=l.size===0?null:l,this.render(a)})}paint(a){let i=a.querySelector(".mk-cards"),e=a.querySelector(".mk-count");if(!i||!e)return;let s=this._q.trim().toLowerCase(),n=this._rows.filter(r=>this._bands&&!this._bands.has(m(Number(r.score)).key)?!1:s?`${r.owner||""}/${r.name}`.toLowerCase().includes(s):!0),l={"score-desc":(r,d)=>Number(d.score)-Number(r.score),"score-asc":(r,d)=>Number(r.score)-Number(d.score),name:(r,d)=>String(r.name).localeCompare(String(d.name)),"size-desc":(r,d)=>Number(d.loc||0)-Number(r.loc||0),recent:(r,d)=>String(d.at||"").localeCompare(String(r.at||""))}[this._sort];n=[...n].sort(l||(()=>0));let t=this._rows.length;if(e.textContent=n.length===t?`${t} project${t===1?"":"s"}`:`${n.length} of ${t} projects`,n.length===0){i.innerHTML=`<p class="mk-empty">${c(this.getAttribute("empty-text")||"No project here matches that.")}</p>`;return}let o="";for(let r of n){let d=Number(r.score),u=m(d),f=[];r.loc&&f.push(`${_(Number(r.loc))} lines`),Number(r.busFactor)===1&&f.push("bus factor 1"),r.day&&f.push(`measured ${r.day}`),r.lastPublished&&r.lastPublished!==r.day&&f.push(`last published ${r.lastPublished}`),o+=`<a class="mk-card" href="${c(r.href||"#")}">`,o+='<span class="mk-card-top">',o+='<span class="mk-card-name">',r.owner&&(o+=`<span class="mk-card-owner">${c(r.owner)}/</span>`),o+=`${c(r.name)}</span>`,r.source==="closed"&&(o+='<span class="mk-card-closed">Closed source</span>'),o+=`<span class="mk-card-score ink-${u.key}">${d.toFixed(1)}</span>`,o+="</span>",o+=`<span class="cai-lens-bar"><span class="cai-lens-fill fill-${u.key}" style="width:${Math.max(2,Math.round(d))}%"></span></span>`,o+=`<span class="mk-card-meta">${c([u.label,...f].join(" \xB7 "))}</span>`,o+="</a>"}i.innerHTML=o}});function _(a){return!Number.isFinite(a)||a<=0?"0":a>=1e6?`${(a/1e6).toFixed(1)}m`:a>=1e3?`${Math.round(a/1e3)}k`:String(a)}
