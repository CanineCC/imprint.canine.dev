...
`,C={rubricVersion:"sample",lenses:[{key:"codeHealth",name:"Code health",alwaysOn:!0},{key:"architecture",name:"Architecture",alwaysOn:!0},{key:"maturity",name:"Maturity",alwaysOn:!0},{key:"productionReadiness",name:"Readiness",alwaysOn:!0},{key:"securityCompliance",name:"Security & Compliance",alwaysOn:!0}],dimensions:[{id:"D1",name:"Cyclomatic Complexity",lens:"codeHealth",evaluator:"tool",family:"dimension",whatItMeasures:"How tangled the control flow is."},{id:"D13",name:"Secret Scanning",lens:"productionReadiness",evaluator:"tool",family:"dimension",whatItMeasures:"Whether secrets have leaked into the code."},{id:"D28",name:"Secrets (history)",lens:"securityCompliance",evaluator:"tool",family:"dimension",whatItMeasures:"Secrets reachable in git history."}]},E={codeHealth:"Code health",architecture:"Architecture",maturity:"Maturity",productionReadiness:"Readiness",securityCompliance:"Security & Compliance",domainModelling:"Domain Modelling",eventDriven:"Event-Driven",eventSourcing:"Event Sourcing",accessibility:"Accessibility",performance:"Performance"};function w(t,r){return(r||[]).find(n=>n.key===t||n.id===t)?.name||E[t]||t||"\u2014"}async function y(t,r){let e=(t||"").trim().replace(/\/$/,"");if(!e)return null;try{let n=await fetch(e+r);return n.ok?await n.json():null}catch{return null}}customElements.define("cai-dimensions",class extends f{#e=null;#t=null;#n=null;#a=!1;async liveLoad(){let t=this.apiBase();if(!t){this.#a=!0;return}let r=await y(t,"/api/rubrics");if(!r||!Array.isArray(r.versions)||r.versions.length===0){this.#a=!0,this.render(this.shadowRoot);return}let e=this.#n||r.latest||r.versions[0],n=await y(t,`/api/rubrics/${encodeURIComponent(e)}/catalog`);if(!n||!Array.isArray(n.dimensions)){this.#a=!0,this.render(this.shadowRoot);return}this.#e=r,this.#t=n,this._live=!0,this.render(this.shadowRoot)}#r(t){this.#n=t,this.liveLoad().catch(()=>{})}render(t){let r=this._live&&this.#t,e=r?this.#t:C,n=e.dimensions||[],i=e.lenses||[],d=n.filter(s=>(s.family||"dimension")==="dimension"),l=n.length-d.length,a=`<style>${b}${g}${v}${$}</style>`;if(a+=x(this),a+='<div class="dx">',a+='<div class="dx-bar">',r&&this.#e){let s=this.#e.versions.map(c=>`<option value="${o(c)}"${c===e.rubricVersion?" selected":""}>${o(c)}</option>`).join("");a+=`<label class="dx-pick">Rubric version <select part="version">${s}</select></label>`,a+='<span class="dx-badge">live from the archive</span>'}else this.#a?a+='<span class="dx-badge sample">sample \u2014 the archive was not reachable</span>':a+='<span class="dx-badge sample">sample \u2014 the live catalogue loads from the archive</span>';if(a+=`<span class="dx-count">${n.length} entries \xB7 ${d.length} with a code \xB7 ${l} meta-dimensions \xB7 ${i.length} lenses</span>`,a+="</div>",i.length){a+='<div class="dx-lenses">';for(let s of i){let c=s.key||s.id,h=d.filter(S=>S.lens===c).length,m=s.alwaysOn===!0||s.always===!0;a+='<div class="dx-lens">',a+=`<div class="dx-lens-name">${o(s.name||w(c,i))}</div>`,a+=`<div class="dx-lens-meta">${m?"always on":"conditional"}</div>`,a+=`<div class="dx-lens-count">${h} coded dimension${h===1?"":"s"}</div>`,a+="</div>"}a+="</div>"}let k=[...d].sort((s,c)=>{let h=parseInt(String(s.id).replace(/\D/g,""),10)||0,m=parseInt(String(c.id).replace(/\D/g,""),10)||0;return h-m});a+='<div class="dx-tablewrap"><table class="dx-t">',a+="<thead><tr><th>Code</th><th>Dimension</th><th>What it measures</th><th>Lens</th><th>Evaluator</th></tr></thead><tbody>";for(let s of k){let c=(s.evaluator||"").toLowerCase();a+="<tr>",a+=`<td class="dx-code">${o(s.id)}</td>`,a+=`<td class="dx-name">${o(s.name||"")}</td>`,a+=`<td class="dx-what">${o(s.whatItMeasures||"")}</td>`,a+=`<td class="dx-lensc">${o(w(s.lens,i))}</td>`,a+=`<td><span class="dx-ev ${c==="tool"?"tool":""}">${o(c||"\u2014")}</span></td>`,a+="</tr>"}if(a+="</tbody></table></div>",r){let s=o(e.rubricVersion),c=o(this.apiBase().replace(/\/$/,""));a+=`<p class="dx-foot">Read verbatim from the published catalogue <code>${s}</code>. Rubrics are immutable, so a score always names the one it was computed under \u2014 pick that version above to read the definitions it was scored against. This catalogue as JSON: <a href="${c}/api/rubrics/${s}/catalog">${s}/catalog</a>.</p>`}a+="</div>",t.innerHTML=a;let p=t.querySelector("select");p&&p.addEventListener("change",s=>this.#r(s.target.value))}});
