...
`;function s(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function d(e){if(e==null||e==="")return"";let n=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,t="",r=0,o;for(;(o=n.exec(e))!==null;){o.index>r&&(t+=s(e.slice(r,o.index)));let a=o[0];if(a.startsWith("**"))t+=`<strong>${s(a.slice(2,-2))}</strong>`;else if(a.startsWith("`"))t+=`<code>${s(a.slice(1,-1))}</code>`;else{let i=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(a);i?t+=`<a href="${s(i[2])}">${s(i[1])}</a>`:t+=s(a)}r=o.index+a.length}return r<e.length&&(t+=s(e.slice(r))),t}var p=`
...
`;function m(e){let n=e.getAttribute("kicker"),t=e.getAttribute("heading"),r=e.getAttribute("lede");if(!n&&!t&&!r)return"";let o='<div class="mk-section-head">';return n&&(o+=`<span class="mk-kicker">${s(n)}</span>`),t&&(o+=`<h2>${d(t)}</h2>`),r&&(o+=`<p>${d(r)}</p>`),o+="</div>",o}var h=`
