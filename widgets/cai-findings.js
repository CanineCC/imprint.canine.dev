...
`;function i(n){return String(n??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function d(n){if(n==null||n==="")return"";let r=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,e="",a=0,t;for(;(t=r.exec(n))!==null;){t.index>a&&(e+=i(n.slice(a,t.index)));let s=t[0];if(s.startsWith("**"))e+=`<strong>${i(s.slice(2,-2))}</strong>`;else if(s.startsWith("`"))e+=`<code>${i(s.slice(1,-1))}</code>`;else{let c=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(s);c?e+=`<a href="${i(c[2])}">${i(c[1])}</a>`:e+=i(s)}a=t.index+s.length}return a<n.length&&(e+=i(n.slice(a))),e}var u=`
...
`;function g(n){let r=n.getAttribute("kicker"),e=n.getAttribute("heading"),a=n.getAttribute("lede");if(!r&&!e&&!a)return"";let t='<div class="mk-section-head">';return r&&(t+=`<span class="mk-kicker">${i(r)}</span>`),e&&(t+=`<h2>${d(e)}</h2>`),a&&(t+=`<p>${d(a)}</p>`),t+="</div>",t}var b=`
...
`;customElements.define("cai-findings",class extends f{async liveLoad(){let n=this.apiBase();if(!n)return;let r=await k(n),e=r&&r.findings;if(!e||!Array.isArray(e.findings)||e.findings.length===0)return;let a=n.replace(/\/$/,""),t={...e,reportUrl:e.reportUrl?/^https?:\/\//i.test(e.reportUrl)?e.reportUrl:a+e.reportUrl:""};this._live=[t],this.render(this.shadowRoot)}render(n){let r=this._live||S,e=`<style>${$}</style>`;e+=g(this),e+='<div class="mk-findings">';for(let t of r){let s=(t.findings||[]).filter(o=>o&&o.title);if(s.length===0)continue;let c=t.shown!=null?t.shown:s.length,m=t.total!=null?t.total:s.length,p=t.owner||"",v=t.name||t.repo||"";e+='<div class="mk-find-repo">',e+='<div class="mk-find-head">',e+=`<span class="mk-find-repo-name"><strong>${i(v)}</strong>`,p&&(e+=`<span> by ${i(p)}</span>`),e+="</span>",e+=`<span class="mk-find-count">showing ${c} of ${m}</span>`,e+="</div>",e+='<ul class="mk-find-list">';for(let o of s){let w=o.lensLabel||o.lens||"Architecture";e+='<li class="mk-find-item">',e+=`<span class="mk-find-lens">${i(w)}</span>`,e+='<div class="mk-find-body">',e+=`<p class="mk-find-title">${i(o.title||"")}</p>`,o.file&&(e+=`<p class="mk-find-loc">${i(o.file)}${o.line?":"+i(String(o.line)):""}</p>`),e+="</div></li>"}e+="</ul>";let l=t.more!=null?t.more:Math.max(0,m-c);l>0&&t.reportUrl?e+=`<p class="mk-find-more"><a href="${i(t.reportUrl)}" target="_blank" rel="noopener">+ ${l} more in the full report \u2192</a></p>`:l>0&&(e+=`<p class="mk-find-more">+ ${l} more in the full report</p>`),e+="</div>"}e+="</div>";let a=this.getAttribute("footnote");a&&(e+=`<p class="mk-grid-foot">${d(a)}</p>`),n.innerHTML=e}});
