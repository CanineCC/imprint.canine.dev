...
`;var y=class extends HTMLElement{#e;connectedCallback(){this.shadowRoot||this.attachShadow({mode:"open"}),this.#a(),this.render(this.shadowRoot),typeof this.liveLoad=="function"&&Promise.resolve(this.liveLoad()).catch(()=>{}),this.#e=new MutationObserver(()=>{let t=this.dataset.theme;this.#a(),this.dataset.theme!==t&&this.render(this.shadowRoot)}),this.#e.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}apiBase(){return(this.getAttribute("api-base")||"").trim()}disconnectedCallback(){this.#e?.disconnect()}#a(){let t=document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.dataset.theme=t;let a=(this.getAttribute("brand")||"").trim().toLowerCase();a==="assay"||a==="cai"||a==="watchdog"?this.dataset.brand=a:delete this.dataset.brand}json(t,a){let n=this.getAttribute(t);if(n==null||n.trim()==="")return a;try{return JSON.parse(n)}catch{return a}}};var d=[{label:"Critical",key:"critical",floor:0},{label:"Weak",key:"poor",floor:25},{label:"Adequate",key:"fair",floor:50},{label:"Strong",key:"healthy",floor:70},{label:"Exemplary",key:"exemplary",floor:90}];function k(e){return e>=90?d[4]:e>=70?d[3]:e>=50?d[2]:e>=25?d[1]:d[0]}function N(e){let t=Math.max(0,Math.min(100,e)),a=d.length-1;for(;a>0&&t<d[a].floor;)a--;let r=(a<d.length-1?d[a+1].floor:100)-d[a].floor,o=r>0?Math.max(0,Math.min(1,(t-d[a].floor)/r)):.5;return{leftPct:a*20+o*20,third:o<1/3?0:o<2/3?1:2,key:d[a].key}}var j=0;function L(){return"s"+(j++).toString(36)}function U(e,{variant:t="diamond",caps:a=!1,className:n}={}){let r=N(e),o=k(e),l=`${r.leftPct.toFixed(2)}%`,u=d.map(h=>`<i class="seg-${h.key}"></i>`).join(""),i;t==="diamond"?i=`<div class="cai-mk cai-diamond" style="left:${l};--dia:var(--band-${o.key})"><span class="cai-diamond-foot"></span></div>`:i=`<div class="cai-mk cai-pin" style="left:${l}"><span class="cai-pin-badge">${Math.round(e)}</span><span class="cai-pin-line"></span><span class="cai-pin-foot"></span></div>`;let s=a?'<div class="cai-caps"><span>Worst</span><span>Best</span></div>':"";return`<div class="${"cai-ladder"+(a?"":" compact")+(n?` ${n}`:"")}"><div class="cai-rail" role="img" aria-label="${Math.round(e)} of 100 on a fixed worst-to-best scale"><div class="cai-segs">${u}</div>${i}</div>${s}</div>`}function O(e){let t=e[0],a=e[e.length-1],n=Math.round(a)-Math.round(t);return`Trend: ${n>=1?`improving (up ${n})`:n<=-1?`declining (down ${-n})`:"steady"} over the last ${e.length} scans.`}function P(e){let t=L(),a=320,n=36,r=4,o=e.length,l=Math.min(...e),u=Math.max(...e),i=Math.max(0,l-3),s=Math.min(100,u+3),p=s-i||1,h=c=>c/(o-1)*(a-2*r)+r,T=c=>r+(1-(c-i)/p)*(n-2*r),b=e.map((c,m)=>[h(m),T(c)]),$=`M ${b[0][0].toFixed(1)} ${b[0][1].toFixed(1)}`;for(let c=0;c<o-1;c++){let m=b[Math.max(0,c-1)],w=b[c],x=b[c+1],S=b[Math.min(o-1,c+2)],A=[w[0]+(x[0]-m[0])/6,w[1]+(x[1]-m[1])/6],C=[x[0]-(S[0]-w[0])/6,x[1]-(S[1]-w[1])/6];$+=` C ${A[0].toFixed(1)} ${A[1].toFixed(1)}, ${C[0].toFixed(1)} ${C[1].toFixed(1)}, ${x[0].toFixed(1)} ${x[1].toFixed(1)}`}let g=[],v=k(i).key;g.push({at:0,key:v});for(let c of d)if(c.floor>i&&c.floor<s){let m=(c.floor-i)/p;g.push({at:m,key:v}),v=c.key,g.push({at:m,key:v})}g.push({at:1,key:v});let D=g.map(c=>`<stop offset="${(c.at*100).toFixed(1)}%" stop-color="var(--band-${c.key})"></stop>`).join("");return`<svg class="cai-spark" viewBox="0 0 ${a} ${n}" preserveAspectRatio="none" aria-hidden="true" focusable="false"><defs><linearGradient id="caisg-${t}" gradientUnits="userSpaceOnUse" x1="0" y1="${n-r}" x2="0" y2="${r}">${D}</linearGradient></defs><path d="${$}" fill="none" stroke="url(#caisg-${t})" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"></path></svg><span class="sr-only">${f(O(e))}</span>`}function B(e){let t=Math.max(0,Math.min(100,Number(e.score)||0)),a=k(t),n=Array.isArray(e.series)&&e.series.length>=2?e.series:null,r=e.arcFirst!=null&&e.arcBest!=null&&Number.isFinite(Number(e.arcFirst))&&Number.isFinite(Number(e.arcBest)),o=r?Math.round(Number(e.arcBest))-Math.round(Number(e.arcFirst)):0,l=(e.lenses||[]).filter(s=>s&&s.label),u=(e.rows||[]).filter(s=>s&&s.label),i="";if(e.sealText&&(i+=`<span class="cai-seal">${f(e.sealText)}</span>`),i+=`<div class="cai-top"><span class="cai-name"><span class="cai-repo">${f(e.name)}</span>`+(e.owner?`<span class="cai-owner">by ${f(e.owner)}</span>`:"")+`</span><span class="cai-chip band-${a.key}">${f(a.label)}</span></div>`,i+=`<div class="cai-scoreline"><span class="cai-cai">CAI</span><span class="cai-score ink-${a.key}">${Math.round(t)}</span><span class="cai-unit"> / 100</span></div>`,i+=U(t,{variant:"diamond"}),n&&(i+=P(n)),r&&(i+=`<div class="cai-arc"><span class="cai-arc-from">${Math.round(Number(e.arcFirst))}</span><span class="cai-arc-arrow" aria-hidden="true">\u2192</span><span class="cai-arc-to ink-${a.key}">${Math.round(Number(e.arcBest))}</span>`+(o>=1?`<span class="cai-arc-up">\u2191 +${o}</span>`:"")+"</div>"),l.length>0){i+='<div class="cai-lenses">';for(let s of l){let p=s.value==null?null:Number(s.value),h=p==null?null:k(p);i+=`<div class="cai-lens"><span class="cai-lens-name">${f(s.label)}</span>`,i+='<span class="cai-lens-bar">',p!=null&&h&&(i+=`<span class="cai-lens-fill fill-${h.key}" style="width:${Math.max(2,Math.round(p))}%"></span>`),i+="</span>",p==null||!h?i+='<span class="cai-lens-num cai-muted">\u2014</span>':i+=`<span class="cai-lens-num ink-${h.key}">${Math.round(p)}</span>`,i+="</div>"}i+="</div>"}if(u.length>0){i+='<div class="cai-rows">';for(let s of u)i+=`<div class="cai-row"><span>${f(s.label)}</span><b class="${s.mono?"mono":""}">${f(s.value)}</b></div>`;i+="</div>"}return i}var R=`
...
`;function z(e){if(!e||!e.name||e.score==null)return null;let t;return Array.isArray(e.series)?t=e.series.map(a=>Number(a)).filter(a=>Number.isFinite(a)):t=String(e.series||"").split(",").map(a=>Number(a.trim())).filter(a=>Number.isFinite(a)),{name:e.name,owner:e.owner||void 0,score:Number(e.score),series:t.length>=2?t:void 0,arcFirst:e.arcFirst??null,arcBest:e.arcBest??null,lenses:(e.lenses||[]).filter(a=>a&&a.label).map(a=>({label:a.label,value:a.value??null})),rows:(e.rows||[]).filter(a=>a&&a.label).map(a=>({label:a.label,value:a.value||"",mono:!!a.mono})),href:e.href||void 0,sealText:e.sealText||void 0}}var G=[["codeHealth","Code health"],["architecture","Architecture"],["maturity","Maturity"],["productionReadiness","Readiness"],["securityCompliance","Security"]];function W(e){return String(e).replace(/\B(?=(\d{3})+(?!\d))/g,",")}function X(e){let t=e.publishedAt||e.lastUpdated,a="";if(t){let o=new Date(t);Number.isNaN(o.getTime())||(a=o.toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"}))}let n=e.productionLoc>0?`${W(e.productionLoc)} lines`:"",r=[a,n].filter(Boolean);return r.length?r.join(" \xB7 "):null}function H(e){if(!e)return null;let t=e.bestScore!=null?e.bestScore:e.headlineScore;if(t==null)return null;let a=Array.isArray(e.series)&&e.series.length>=2,n=G.map(([l,u])=>({label:u,value:e[l]==null?null:Number(e[l])})).filter(l=>l.value!=null),r=[],o=X(e);return o&&r.push({label:"Measured",value:o}),e.costApprox&&r.push({label:"Rebuild cost",value:e.costApprox}),e.busFactor>0&&e.authorCount>0&&r.push({label:"Bus factor",value:`${e.busFactor} of ${e.authorCount} devs`}),{name:e.name,owner:e.owner||void 0,score:Number(t),series:a?e.series.map(Number):void 0,arcFirst:a?e.firstScore:null,arcBest:a?e.bestScore:null,lenses:n,rows:r}}function _(e,t){if(!e)return"";let a=e.bestRunId||e.BestRunId,n=e.owner||e.Owner,r=e.name||e.Name;return!a||!n||!r?"":(t||"").trim().replace(/\/$/,"")+"/api/oss/"+encodeURIComponent(n)+"/"+encodeURIComponent(r)+"/report?run="+encodeURIComponent(a)}var E=new Map;function q(e,t){return e+" "+(t||"")}function I(e,t){let a=(e||"").trim().replace(/\/$/,"");if(!a)return Promise.resolve(null);let n=q(a,t),r=E.get(n);if(r)return r;let o=t?"?cohort="+encodeURIComponent(t):"";return r=(async()=>{try{let l=await fetch(a+"/api/public/showcase"+o);return l.ok?await l.json():null}catch{return null}})(),E.set(n,r),r}var J={name:"checkout-service",owner:"acme",score:62,series:[45,48,47,52,55,58,60,62],arcFirst:45,arcBest:62,lenses:[{label:"Code health",value:68},{label:"Architecture",value:55},{label:"Maturity",value:63},{label:"Readiness",value:52},{label:"Security",value:71}],rows:[{label:"Measured",value:"1 July 2026 \xB7 4.2M lines"},{label:"Reproducible fingerprint",value:"a3f9\u2026e021",mono:!0},{label:"Shared with",value:"3 parties"}]},K=M+F+R+`
:host { display: block; }
.cai-card-cap { margin-top: 0.85rem; }
`;customElements.define("cai-score-card",class extends y{async liveLoad(){let e=this.apiBase();if(!e)return;let t=await I(e),a=t&&t.hero;if(!a)return;let n=H(a);if(!n)return;let r=_(a,e);r&&(n.href=r),this._live=n,this.render(this.shadowRoot)}render(e){let t;if(this._live)t={...this._live};else{let h=z(this.json("card",null));t=h?{...h}:{...J}}let a=this.getAttribute("name"),n=this.getAttribute("owner"),r=this.getAttribute("score"),o=this.getAttribute("seal-text"),l=this.getAttribute("href");this._live||(a!=null&&a!==""&&(t.name=a),n!=null&&n!==""&&(t.owner=n)),r!=null&&r!==""&&Number.isFinite(Number(r))&&(t.score=Number(r)),o!=null&&o!==""&&(t.sealText=o),l!=null&&l!==""&&(t.href=l);let u=this._live?null:this.getAttribute("caption"),i=t.href?"a":"div",s=t.href?` href="${f(t.href)}"`:"",p=`<style>${K}</style>`;p+=`<${i} class="cai-card"${s}>${B(t)}</${i}>`,u&&(p+=`<p class="cai-card-cap">${f(u)}</p>`),e.innerHTML=p}});
