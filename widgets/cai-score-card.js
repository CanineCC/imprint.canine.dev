...
`;var w=class extends HTMLElement{#e;connectedCallback(){this.shadowRoot||this.attachShadow({mode:"open"}),this.#a(),this.render(this.shadowRoot),typeof this.liveLoad=="function"&&Promise.resolve(this.liveLoad()).catch(()=>{}),this.#e=new MutationObserver(()=>{let t=this.dataset.theme;this.#a(),this.dataset.theme!==t&&this.render(this.shadowRoot)}),this.#e.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}apiBase(){return(this.getAttribute("api-base")||"").trim()}disconnectedCallback(){this.#e?.disconnect()}#a(){let t=document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.dataset.theme=t;let a=(this.getAttribute("brand")||"").trim().toLowerCase();a==="assay"||a==="cai"||a==="watchdog"?this.dataset.brand=a:delete this.dataset.brand}json(t,a){let r=this.getAttribute(t);if(r==null||r.trim()==="")return a;try{return JSON.parse(r)}catch{return a}}};var d=[{label:"Critical",key:"critical",floor:0},{label:"Weak",key:"poor",floor:25},{label:"Adequate",key:"fair",floor:50},{label:"Strong",key:"healthy",floor:70},{label:"Exemplary",key:"exemplary",floor:90}];function y(e){return e>=90?d[4]:e>=70?d[3]:e>=50?d[2]:e>=25?d[1]:d[0]}function F(e){let t=Math.max(0,Math.min(100,e)),a=d.length-1;for(;a>0&&t<d[a].floor;)a--;let n=(a<d.length-1?d[a+1].floor:100)-d[a].floor,o=n>0?Math.max(0,Math.min(1,(t-d[a].floor)/n)):.5;return{leftPct:a*20+o*20,third:o<1/3?0:o<2/3?1:2,key:d[a].key}}var j=0;function D(){return"s"+(j++).toString(36)}function I(e,{variant:t="diamond",caps:a=!1,className:r}={}){let n=F(e),o=y(e),l=`${n.leftPct.toFixed(2)}%`,p=d.map(h=>`<i class="seg-${h.key}"></i>`).join(""),i;t==="diamond"?i=`<div class="cai-mk cai-diamond" style="left:${l};--dia:var(--band-${o.key})"><span class="cai-diamond-foot"></span></div>`:i=`<div class="cai-mk cai-pin" style="left:${l}"><span class="cai-pin-badge">${Math.round(e)}</span><span class="cai-pin-line"></span><span class="cai-pin-foot"></span></div>`;let s=a?'<div class="cai-caps"><span>Worst</span><span>Best</span></div>':"";return`<div class="${"cai-ladder"+(a?"":" compact")+(r?` ${r}`:"")}"><div class="cai-rail" role="img" aria-label="${Math.round(e)} of 100 on a fixed worst-to-best scale"><div class="cai-segs">${p}</div>${i}</div>${s}</div>`}function G(e){let t=e[0],a=e[e.length-1],r=Math.round(a)-Math.round(t);return`Trend: ${r>=1?`improving (up ${r})`:r<=-1?`declining (down ${-r})`:"steady"} over the last ${e.length} scans.`}function O(e){let t=D(),a=320,r=36,n=4,o=e.length,l=Math.min(...e),p=Math.max(...e),i=Math.max(0,l-3),s=Math.min(100,p+3),u=s-i||1,h=c=>c/(o-1)*(a-2*n)+n,_=c=>n+(1-(c-i)/u)*(r-2*n),b=e.map((c,m)=>[h(m),_(c)]),$=`M ${b[0][0].toFixed(1)} ${b[0][1].toFixed(1)}`;for(let c=0;c<o-1;c++){let m=b[Math.max(0,c-1)],k=b[c],g=b[c+1],S=b[Math.min(o-1,c+2)],A=[k[0]+(g[0]-m[0])/6,k[1]+(g[1]-m[1])/6],C=[g[0]-(S[0]-k[0])/6,g[1]-(S[1]-k[1])/6];$+=` C ${A[0].toFixed(1)} ${A[1].toFixed(1)}, ${C[0].toFixed(1)} ${C[1].toFixed(1)}, ${g[0].toFixed(1)} ${g[1].toFixed(1)}`}let x=[],v=y(i).key;x.push({at:0,key:v});for(let c of d)if(c.floor>i&&c.floor<s){let m=(c.floor-i)/u;x.push({at:m,key:v}),v=c.key,x.push({at:m,key:v})}x.push({at:1,key:v});let U=x.map(c=>`<stop offset="${(c.at*100).toFixed(1)}%" stop-color="var(--band-${c.key})"></stop>`).join("");return`<svg class="cai-spark" viewBox="0 0 ${a} ${r}" preserveAspectRatio="none" aria-hidden="true" focusable="false"><defs><linearGradient id="caisg-${t}" gradientUnits="userSpaceOnUse" x1="0" y1="${r-n}" x2="0" y2="${n}">${U}</linearGradient></defs><path d="${$}" fill="none" stroke="url(#caisg-${t})" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"></path></svg><span class="sr-only">${f(G(e))}</span>`}function H(e){let t=Math.max(0,Math.min(100,Number(e.score)||0)),a=y(t),r=Array.isArray(e.series)&&e.series.length>=2?e.series:null,n=e.arcFirst!=null&&e.arcBest!=null&&Number.isFinite(Number(e.arcFirst))&&Number.isFinite(Number(e.arcBest)),o=n?Math.round(Number(e.arcBest))-Math.round(Number(e.arcFirst)):0,l=(e.lenses||[]).filter(s=>s&&s.label),p=(e.rows||[]).filter(s=>s&&s.label),i="";if(e.sealText&&(i+=`<span class="cai-seal">${f(e.sealText)}</span>`),i+=`<div class="cai-top"><span class="cai-name"><span class="cai-repo">${f(e.name)}</span>`+(e.owner?`<span class="cai-owner">by ${f(e.owner)}</span>`:"")+`</span><span class="cai-chip band-${a.key}">${f(a.label)}</span></div>`,i+=`<div class="cai-scoreline"><span class="cai-cai">CAI</span><span class="cai-score ink-${a.key}">${Math.round(t)}</span><span class="cai-unit"> / 100</span></div>`,i+=I(t,{variant:"diamond"}),r&&(i+=O(r)),n&&(i+=`<div class="cai-arc"><span class="cai-arc-from">${Math.round(Number(e.arcFirst))}</span><span class="cai-arc-arrow" aria-hidden="true">\u2192</span><span class="cai-arc-to ink-${a.key}">${Math.round(Number(e.arcBest))}</span>`+(o>=1?`<span class="cai-arc-up">\u2191 +${o}</span>`:"")+"</div>"),l.length>0){i+='<div class="cai-lenses">';for(let s of l){let u=s.value==null?null:Number(s.value),h=u==null?null:y(u);i+=`<div class="cai-lens"><span class="cai-lens-name">${f(s.label)}</span>`,i+='<span class="cai-lens-bar">',u!=null&&h&&(i+=`<span class="cai-lens-fill fill-${h.key}" style="width:${Math.max(2,Math.round(u))}%"></span>`),i+="</span>",u==null||!h?i+='<span class="cai-lens-num cai-muted">\u2014</span>':i+=`<span class="cai-lens-num ink-${h.key}">${Math.round(u)}</span>`,i+="</div>"}i+="</div>"}if(p.length>0){i+='<div class="cai-rows">';for(let s of p)i+=`<div class="cai-row"><span>${f(s.label)}</span><b class="${s.mono?"mono":""}">${f(s.value)}</b></div>`;i+="</div>"}return i}var L=`
...
`;function R(e){if(!e||!e.name||e.score==null)return null;let t;return Array.isArray(e.series)?t=e.series.map(a=>Number(a)).filter(a=>Number.isFinite(a)):t=String(e.series||"").split(",").map(a=>Number(a.trim())).filter(a=>Number.isFinite(a)),{name:e.name,owner:e.owner||void 0,score:Number(e.score),series:t.length>=2?t:void 0,arcFirst:e.arcFirst??null,arcBest:e.arcBest??null,lenses:(e.lenses||[]).filter(a=>a&&a.label).map(a=>({label:a.label,value:a.value??null})),rows:(e.rows||[]).filter(a=>a&&a.label).map(a=>({label:a.label,value:a.value||"",mono:!!a.mono})),href:e.href||void 0,sealText:e.sealText||void 0}}var q=[["codeHealth","Code health"],["architecture","Architecture"],["maturity","Maturity"],["productionReadiness","Readiness"],["securityCompliance","Security"]];function W(e){return String(e).replace(/\B(?=(\d{3})+(?!\d))/g,",")}function J(e){let t=e.publishedAt||e.lastUpdated,a="";if(t){let o=new Date(t);Number.isNaN(o.getTime())||(a=o.toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"}))}let r=e.productionLoc>0?`${W(e.productionLoc)} lines`:"",n=[a,r].filter(Boolean);return n.length?n.join(" \xB7 "):null}function z(e){if(!e)return null;let t=e.bestScore!=null?e.bestScore:e.headlineScore;if(t==null)return null;let a=Array.isArray(e.series)&&e.series.length>=2,r=q.map(([p,i])=>({label:i,value:e[p]==null?null:Number(e[p])})).filter(p=>p.value!=null),n=[],o=J(e);o&&n.push({label:"Measured",value:o}),e.costApprox&&n.push({label:"Rebuild cost",value:e.costApprox}),e.busFactor>0&&e.authorCount>0&&n.push({label:"Bus factor",value:`${e.busFactor} of ${e.authorCount} devs`});let l=typeof e.display=="string"&&e.display.length>0&&e.display!==(e.owner?e.owner+"/"+e.name:e.name);return{name:l?e.display:e.name,owner:l?void 0:e.owner||void 0,score:Number(t),series:a?e.series.map(Number):void 0,arcFirst:a?e.firstScore:null,arcBest:a?e.bestScore:null,lenses:r,rows:n}}function B(e,t){if(!e)return"";let a=e.bestRunId||e.BestRunId,r=e.owner||e.Owner,n=e.name||e.Name;return!a||!r||!n?"":(t||"").trim().replace(/\/$/,"")+"/api/oss/"+encodeURIComponent(r)+"/"+encodeURIComponent(n)+"/report?run="+encodeURIComponent(a)}var E=new Map;function X(e,t,a){let r=(e||"").trim().replace(/\/$/,"");if(!r)return Promise.resolve(a);let n=r+" "+t,o=E.get(n);return o||(o=(async()=>{try{let l=await fetch(r+t);return l.ok?await l.json():a}catch{return a}})(),E.set(n,o),o)}async function P(e){let t=await X(e,"/api/oss",[]);return Array.isArray(t)?t:[]}function T(e){if(!Array.isArray(e)||e.length===0)return null;let t=r=>r.bestScore!=null?Number(r.bestScore):Number(r.headlineScore)||0,a=null;for(let r of e)(a===null||t(r)>t(a)||t(r)===t(a)&&(Number(r.productionLoc)||0)>(Number(a.productionLoc)||0))&&(a=r);return a}var K={name:"checkout-service",owner:"acme",score:62,series:[45,48,47,52,55,58,60,62],arcFirst:45,arcBest:62,lenses:[{label:"Code health",value:68},{label:"Architecture",value:55},{label:"Maturity",value:63},{label:"Readiness",value:52},{label:"Security",value:71}],rows:[{label:"Measured",value:"1 July 2026 \xB7 4.2M lines"},{label:"Reproducible fingerprint",value:"a3f9\u2026e021",mono:!0},{label:"Shared with",value:"3 parties"}]},Q=M+N+L+`
:host { display: block; }
.cai-card-cap { margin-top: 0.85rem; }
`;customElements.define("cai-score-card",class extends w{async liveLoad(){let e=this.apiBase();if(!e)return;let t=await P(e),a=T(t);if(!a)return;let r=z(a);if(!r)return;let n=B(a,e);n&&(r.href=n),this._live=r,this.render(this.shadowRoot)}render(e){let t;if(this._live)t={...this._live};else{let h=R(this.json("card",null));t=h?{...h}:{...K}}let a=this.getAttribute("name"),r=this.getAttribute("owner"),n=this.getAttribute("score"),o=this.getAttribute("seal-text"),l=this.getAttribute("href");this._live||(a!=null&&a!==""&&(t.name=a),r!=null&&r!==""&&(t.owner=r)),n!=null&&n!==""&&Number.isFinite(Number(n))&&(t.score=Number(n)),o!=null&&o!==""&&(t.sealText=o),l!=null&&l!==""&&(t.href=l);let p=this._live?null:this.getAttribute("caption"),i=t.href?"a":"div",s=t.href?` href="${f(t.href)}"`:"",u=`<style>${Q}</style>`;u+=`<${i} class="cai-card"${s}>${H(t)}</${i}>`,p&&(u+=`<p class="cai-card-cap">${f(p)}</p>`),e.innerHTML=u}});
