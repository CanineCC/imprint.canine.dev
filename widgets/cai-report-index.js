...
`;function l(n){return String(n??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function g(n){if(n==null||n==="")return"";let e=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,t="",a=0,r;for(;(r=e.exec(n))!==null;){r.index>a&&(t+=l(n.slice(a,r.index)));let s=r[0];if(s.startsWith("**"))t+=`<strong>${l(s.slice(2,-2))}</strong>`;else if(s.startsWith("`"))t+=`<code>${l(s.slice(1,-1))}</code>`;else{let i=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(s);i?t+=`<a href="${l(i[2])}">${l(i[1])}</a>`:t+=l(s)}a=r.index+s.length}return a<n.length&&(t+=l(n.slice(a))),t}var b=`
...
`;function y(n){let e=n.getAttribute("kicker"),t=n.getAttribute("heading"),a=n.getAttribute("lede");if(!e&&!t&&!a)return"";let r='<div class="mk-section-head">';return e&&(r+=`<span class="mk-kicker">${l(e)}</span>`),t&&(r+=`<h2>${g(t)}</h2>`),a&&(r+=`<p>${g(a)}</p>`),r+="</div>",r}var x=`
...
`,C=[["codeHealth","Code Health"],["architecture","Architecture"],["maturity","Maturity"],["productionReadiness","Production Readiness"],["securityCompliance","Security & Compliance"],["domainModelling","Domain Modelling"],["eventDriven","Event-Driven"],["eventSourcing","Event Sourcing"],["accessibility","Accessibility"],["performance","Performance"]],E={csharp:"C#",vbnet:"VB.NET",fsharp:"F#",javascript:"JavaScript",typescript:"TypeScript",php:"PHP"},S=n=>E[n]||n&&n[0].toUpperCase()+n.slice(1),c=n=>String(n??"").replace(/[<>&"]/g,e=>({"<":"&lt;",">":"&gt;","&":"&amp;",'"':"&quot;"})[e]);function A(n){return C.map(([e])=>e).filter(e=>n[e]!==void 0&&n[e]!==null)}function h(n){let e=n.primaryLanguage||null,t=Array.isArray(n.secondaryLanguages)?n.secondaryLanguages:[];return e?[e,...t]:[]}var f=class extends p{static get observedAttributes(){return["api-base","kicker","heading","lede","brand"]}#r(){let e=y(this);return e||'<div class="mk-section-head"><h2>Public surveys</h2><p>Every survey whose owner chose to publish it, with the score exactly as it was measured. The number is reproducible from the evidence and the rubric it was scored under.</p></div>'}#e=[];#t=null;#n=null;render(e){e.innerHTML=`<style>${H}</style>${this.#r()}<div id="body"><p class="mk-empty">Loading the published record\u2026</p></div>`,this.#a(e)}async liveLoad(){let e=await w(this.apiBase());this.#e=Array.isArray(e)?e:[],this.shadowRoot&&this.#a(this.shadowRoot)}#a(e){let t=e.getElementById("body");if(!t)return;if(this.#e.length===0){t.innerHTML='<p class="mk-empty">No published surveys are available right now.</p>';return}let a=this.#e.filter(r=>(!this.#t||h(r).includes(this.#t))&&(!this.#n||A(r).includes(this.#n)));t.innerHTML=this.#s()+this.#i(a)+this.#o(a,e),this.#c(e)}#s(){let e=new Map;for(let i of this.#e)for(let o of h(i))e.set(o,(e.get(o)||0)+1);let t=C.map(([i,o])=>[i,o,this.#e.filter(d=>A(d).includes(i)).length]).filter(([,,i])=>i>0),a=[...e.entries()].sort((i,o)=>o[1]-i[1]||i[0].localeCompare(o[0]));if(a.length===0&&t.length===0)return"";let r=(i,o,d,u,L)=>`<button class="mk-chip" type="button" data-kind="${i}" data-value="${c(o??"")}" aria-pressed="${L}">${c(d)}${u===null?"":` <span class="n">${u}</span>`}</button>`,s='<div class="mk-filters">';if(a.length){s+=`<div class="mk-frow"><span class="mk-flabel">Language</span>${r("lang","","All",null,this.#t===null)}`;for(let[i,o]of a)s+=r("lang",i,S(i),o,this.#t===i);s+="</div>"}if(t.length){s+=`<div class="mk-frow"><span class="mk-flabel">Lens</span>${r("lens","","All",null,this.#n===null)}`;for(let[i,o,d]of t)s+=r("lens",i,o,d,this.#n===i);s+="</div>"}return s+"</div>"}#i(e){return`<p class="mk-count">${this.#t||this.#n?`${e.length} of ${this.#e.length} published surveys`:`${this.#e.length} published surveys`}</p>`}#o(e){if(e.length===0)return'<p class="mk-empty">No published survey matches that combination.</p>';let t=this.apiBase();return`<table class="mk-table">
        <thead><tr><th>Repository</th><th>Languages</th><th>CAI</th><th>Band</th><th></th></tr></thead>
        <tbody>${e.map(r=>{let s=h(r),i=s.length?s.map((d,u)=>`<span class="mk-lang${u===0?" primary":""}">${c(S(d))}</span>`).join(""):'<span class="mk-lang">\u2014</span>',o=k(r,t);return`<tr>
