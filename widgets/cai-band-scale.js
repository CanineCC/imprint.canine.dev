...
`;function l(a){return String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function p(a){if(a==null||a==="")return"";let r=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,e="",t=0,n;for(;(n=r.exec(a))!==null;){n.index>t&&(e+=l(a.slice(t,n.index)));let i=n[0];if(i.startsWith("**"))e+=`<strong>${l(i.slice(2,-2))}</strong>`;else if(i.startsWith("`"))e+=`<code>${l(i.slice(1,-1))}</code>`;else{let c=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(i);c?e+=`<a href="${l(c[2])}">${l(c[1])}</a>`:e+=l(i)}t=n.index+i.length}return t<a.length&&(e+=l(a.slice(t))),e}var h=`
...
`;function b(a){let r=a.getAttribute("kicker"),e=a.getAttribute("heading"),t=a.getAttribute("lede");if(!r&&!e&&!t)return"";let n='<div class="mk-section-head">';return r&&(n+=`<span class="mk-kicker">${l(r)}</span>`),e&&(n+=`<h2>${p(e)}</h2>`),t&&(n+=`<p>${p(t)}</p>`),n+="</div>",n}var x=`
...
`,f=class extends HTMLElement{#e;connectedCallback(){this.shadowRoot||this.attachShadow({mode:"open"}),this.#a(),this.render(this.shadowRoot),typeof this.liveLoad=="function"&&Promise.resolve(this.liveLoad()).catch(()=>{}),this.#e=new MutationObserver(()=>{let r=this.dataset.theme;this.#a(),this.dataset.theme!==r&&this.render(this.shadowRoot)}),this.#e.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}apiBase(){return(this.getAttribute("api-base")||"").trim()}disconnectedCallback(){this.#e?.disconnect()}#a(){let r=document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.dataset.theme=r;let e=(this.getAttribute("brand")||"").trim().toLowerCase();e==="assay"||e==="cai"||e==="watchdog"?this.dataset.brand=e:delete this.dataset.brand}json(r,e){let t=this.getAttribute(r);if(t==null||t.trim()==="")return e;try{return JSON.parse(t)}catch{return e}}};var s=[{label:"Critical",key:"critical",floor:0},{label:"Weak",key:"poor",floor:25},{label:"Adequate",key:"fair",floor:50},{label:"Strong",key:"healthy",floor:70},{label:"Exemplary",key:"exemplary",floor:90}];function g(a){return a>=90?s[4]:a>=70?s[3]:a>=50?s[2]:a>=25?s[1]:s[0]}function v(a){let r=Math.max(0,Math.min(100,a)),e=s.length-1;for(;e>0&&r<s[e].floor;)e--;let n=(e<s.length-1?s[e+1].floor:100)-s[e].floor,i=n>0?Math.max(0,Math.min(1,(r-s[e].floor)/n)):.5;return{leftPct:e*20+i*20,third:i<1/3?0:i<2/3?1:2,key:s[e].key}}function k(a,{variant:r="diamond",caps:e=!1,className:t}={}){let n=v(a),i=g(a),c=`${n.leftPct.toFixed(2)}%`,o=s.map(S=>`<i class="seg-${S.key}"></i>`).join(""),d;r==="diamond"?d=`<div class="cai-mk cai-diamond" style="left:${c};--dia:var(--band-${i.key})"><span class="cai-diamond-foot"></span></div>`:d=`<div class="cai-mk cai-pin" style="left:${c}"><span class="cai-pin-badge">${Math.round(a)}</span><span class="cai-pin-line"></span><span class="cai-pin-foot"></span></div>`;let u=e?'<div class="cai-caps"><span>Worst</span><span>Best</span></div>':"";return`<div class="${"cai-ladder"+(e?"":" compact")+(t?` ${t}`:"")}"><div class="cai-rail" role="img" aria-label="${Math.round(a)} of 100 on a fixed worst-to-best scale"><div class="cai-segs">${o}</div>${d}</div>${u}</div>`}var y=`
...
`;function w(a,{owner:r,name:e}={}){if(!Array.isArray(a)||a.length===0)return null;if(r&&e){let t=a.find(n=>n.owner===r&&n.name===e);if(t)return t}return a.slice().sort((t,n)=>{let i=t.bestScore!=null?t.bestScore:t.headlineScore;return(n.bestScore!=null?n.bestScore:n.headlineScore)-i||(n.productionLoc||0)-(t.productionLoc||0)})[0]}async function $(a,r,e){if(!a)return e;try{let t=await fetch(a.replace(/\/$/,"")+r);return t.ok?await t.json():e}catch{return e}}var A=m+h+x+y+`
...
`;customElements.define("cai-band-scale",class extends f{async liveLoad(){let a=this.apiBase();if(!a)return;let r=this.getAttribute("owner")||"",e=this.getAttribute("name")||"",t=await $(a,"/api/oss",null);if(!Array.isArray(t)||t.length===0)return;let n=w(t,{owner:r,name:e}),i=n&&(n.bestScore!=null?n.bestScore:n.headlineScore);i==null||!Number.isFinite(Number(i))||(this._liveScore=Number(i),this.render(this.shadowRoot))}render(a){let r=this.getAttribute("score"),e=this._liveScore!=null?this._liveScore:r!=null&&r!==""&&Number.isFinite(Number(r))?Number(r):null,t=e!=null,n=this.getAttribute("caption"),i;t?i=k(e,{variant:"pin"}):i=`<div class="cai-ladder compact"><div class="cai-rail" role="img" aria-label="The fixed worst-to-best CAI band scale"><div class="cai-segs">${s.map(u=>`<i class="seg-${u.key}"></i>`).join("")}</div></div></div>`;let c=s.map(d=>`<span class="ink-${d.key}">${d.label}</span>`).join(""),o=`<style>${A}</style>`;o+=b(this),o+='<figure class="mk-bandscale">',o+=`<div class="mk-bandscale-rail${t?" has-pin":""}">`,o+=i,o+='<div class="mk-bandscale-cuts" aria-hidden="true"><span style="left:0%">0</span><span style="left:20%">25</span><span style="left:40%">50</span><span style="left:60%">70</span><span style="left:80%">90</span><span style="left:100%">100</span></div>',o+="</div>",o+=`<div class="mk-bandscale-words">${c}</div>`,n&&(o+=`<figcaption class="mk-bandscale-cap">${p(n)}</figcaption>`),o+="</figure>",a.innerHTML=o}});
