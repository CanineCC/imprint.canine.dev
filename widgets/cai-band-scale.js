...
`;function b(a){let t=a.getAttribute("kicker"),e=a.getAttribute("heading"),n=a.getAttribute("lede");if(!t&&!e&&!n)return"";let i='<div class="mk-section-head">';return t&&(i+=`<span class="mk-kicker">${l(t)}</span>`),e&&(i+=`<h2>${p(e)}</h2>`),n&&(i+=`<p>${p(n)}</p>`),i+="</div>",i}var x=`
...
`;var w=new Map;function N(a,t,e){let n=(a||"").trim().replace(/\/$/,"");if(!n)return Promise.resolve(e);let i=n+" "+t,r=w.get(i);return r||(r=(async()=>{try{let c=await fetch(n+t);return c.ok?await c.json():e}catch{return e}})(),w.set(i,r),r)}async function $(a){let t=await N(a,"/api/oss",[]);return Array.isArray(t)?t:[]}function S(a){if(!Array.isArray(a)||a.length===0)return null;let t=n=>n.bestScore!=null?Number(n.bestScore):Number(n.headlineScore)||0,e=null;for(let n of a)(e===null||t(n)>t(e)||t(n)===t(e)&&(Number(n.productionLoc)||0)>(Number(e.productionLoc)||0))&&(e=n);return e}var C=m+h+x+y+`
