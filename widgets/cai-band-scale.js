...
`;function c(a){return String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function p(a){if(a==null||a==="")return"";let t=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,e="",n=0,r;for(;(r=t.exec(a))!==null;){r.index>n&&(e+=c(a.slice(n,r.index)));let i=r[0];if(i.startsWith("**"))e+=`<strong>${c(i.slice(2,-2))}</strong>`;else if(i.startsWith("`"))e+=`<code>${c(i.slice(1,-1))}</code>`;else{let l=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(i);l?e+=`<a href="${c(l[2])}">${c(l[1])}</a>`:e+=c(i)}n=r.index+i.length}return n<a.length&&(e+=c(a.slice(n))),e}var h=`
...
`;function b(a){let t=a.getAttribute("kicker"),e=a.getAttribute("heading"),n=a.getAttribute("lede");if(!t&&!e&&!n)return"";let r='<div class="mk-section-head">';return t&&(r+=`<span class="mk-kicker">${c(t)}</span>`),e&&(r+=`<h2>${p(e)}</h2>`),n&&(r+=`<p>${p(n)}</p>`),r+="</div>",r}var x=`
...
`,f=class extends HTMLElement{#e;connectedCallback(){this.shadowRoot||this.attachShadow({mode:"open"}),this.#a(),this.render(this.shadowRoot),typeof this.liveLoad=="function"&&Promise.resolve(this.liveLoad()).catch(()=>{}),this.#e=new MutationObserver(()=>{let t=this.dataset.theme;this.#a(),this.dataset.theme!==t&&this.render(this.shadowRoot)}),this.#e.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}apiBase(){return(this.getAttribute("api-base")||"").trim()}disconnectedCallback(){this.#e?.disconnect()}#a(){let t=document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.dataset.theme=t;let e=(this.getAttribute("brand")||"").trim().toLowerCase();e==="assay"||e==="cai"||e==="watchdog"?this.dataset.brand=e:delete this.dataset.brand}json(t,e){let n=this.getAttribute(t);if(n==null||n.trim()==="")return e;try{return JSON.parse(n)}catch{return e}}};var o=[{label:"Critical",key:"critical",floor:0},{label:"Weak",key:"poor",floor:25},{label:"Adequate",key:"fair",floor:50},{label:"Strong",key:"healthy",floor:70},{label:"Exemplary",key:"exemplary",floor:90}];function g(a){return a>=90?o[4]:a>=70?o[3]:a>=50?o[2]:a>=25?o[1]:o[0]}function v(a){let t=Math.max(0,Math.min(100,a)),e=o.length-1;for(;e>0&&t<o[e].floor;)e--;let r=(e<o.length-1?o[e+1].floor:100)-o[e].floor,i=r>0?Math.max(0,Math.min(1,(t-o[e].floor)/r)):.5;return{leftPct:e*20+i*20,third:i<1/3?0:i<2/3?1:2,key:o[e].key}}function k(a,{variant:t="diamond",caps:e=!1,className:n}={}){let r=v(a),i=g(a),l=`${r.leftPct.toFixed(2)}%`,s=o.map(A=>`<i class="seg-${A.key}"></i>`).join(""),d;t==="diamond"?d=`<div class="cai-mk cai-diamond" style="left:${l};--dia:var(--band-${i.key})"><span class="cai-diamond-foot"></span></div>`:d=`<div class="cai-mk cai-pin" style="left:${l}"><span class="cai-pin-badge">${Math.round(a)}</span><span class="cai-pin-line"></span><span class="cai-pin-foot"></span></div>`;let u=e?'<div class="cai-caps"><span>Worst</span><span>Best</span></div>':"";return`<div class="${"cai-ladder"+(e?"":" compact")+(n?` ${n}`:"")}"><div class="cai-rail" role="img" aria-label="${Math.round(a)} of 100 on a fixed worst-to-best scale"><div class="cai-segs">${s}</div>${d}</div>${u}</div>`}var y=`
...
`;function w(a){if(!a)return!1;let t=a.sourceUrl||a.SourceUrl,e=a.bestRunId||a.BestRunId;return!!(t&&e)}function $(a,{owner:t,name:e}={}){if(!Array.isArray(a)||a.length===0)return null;let n=a.filter(w);if(t&&e){let r=n.find(i=>i.owner===t&&i.name===e);if(r)return r}return C(a)[0]||null}function C(a){return a.filter(w).slice().sort((t,e)=>{let n=t.bestScore!=null?t.bestScore:t.headlineScore;return(e.bestScore!=null?e.bestScore:e.headlineScore)-n||(e.productionLoc||0)-(t.productionLoc||0)})}async function S(a,t,e){if(!a)return e;try{let n=await fetch(a.replace(/\/$/,"")+t);return n.ok?await n.json():e}catch{return e}}var N=m+h+x+y+`
...
`;customElements.define("cai-band-scale",class extends f{async liveLoad(){let a=this.apiBase();if(!a)return;let t=this.getAttribute("owner")||"",e=this.getAttribute("name")||"",n=await S(a,"/api/oss",null);if(!Array.isArray(n)||n.length===0)return;let r=$(n,{owner:t,name:e}),i=r&&(r.bestScore!=null?r.bestScore:r.headlineScore);i==null||!Number.isFinite(Number(i))||(this._liveScore=Number(i),this.render(this.shadowRoot))}render(a){let t=this.getAttribute("score"),e=this._liveScore!=null?this._liveScore:t!=null&&t!==""&&Number.isFinite(Number(t))?Number(t):null,n=e!=null,r=this.getAttribute("caption"),i;n?i=k(e,{variant:"pin"}):i=`<div class="cai-ladder compact"><div class="cai-rail" role="img" aria-label="The fixed worst-to-best CAI band scale"><div class="cai-segs">${o.map(u=>`<i class="seg-${u.key}"></i>`).join("")}</div></div></div>`;let l=o.map(d=>`<span class="ink-${d.key}">${d.label}</span>`).join(""),s=`<style>${N}</style>`;s+=b(this),s+='<figure class="mk-bandscale">',s+=`<div class="mk-bandscale-rail${n?" has-pin":""}">`,s+=i,s+='<div class="mk-bandscale-cuts" aria-hidden="true"><span style="left:0%">0</span><span style="left:20%">25</span><span style="left:40%">50</span><span style="left:60%">70</span><span style="left:80%">90</span><span style="left:100%">100</span></div>',s+="</div>",s+=`<div class="mk-bandscale-words">${l}</div>`,r&&(s+=`<figcaption class="mk-bandscale-cap">${p(r)}</figcaption>`),s+="</figure>",a.innerHTML=s}});
