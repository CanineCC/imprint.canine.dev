...
`;function l(a){return String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function p(a){if(a==null||a==="")return"";let t=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,e="",n=0,i;for(;(i=t.exec(a))!==null;){i.index>n&&(e+=l(a.slice(n,i.index)));let r=i[0];if(r.startsWith("**"))e+=`<strong>${l(r.slice(2,-2))}</strong>`;else if(r.startsWith("`"))e+=`<code>${l(r.slice(1,-1))}</code>`;else{let c=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(r);c?e+=`<a href="${l(c[2])}">${l(c[1])}</a>`:e+=l(r)}n=i.index+r.length}return n<a.length&&(e+=l(a.slice(n))),e}var h=`
...
`;function b(a){let t=a.getAttribute("kicker"),e=a.getAttribute("heading"),n=a.getAttribute("lede");if(!t&&!e&&!n)return"";let i='<div class="mk-section-head">';return t&&(i+=`<span class="mk-kicker">${l(t)}</span>`),e&&(i+=`<h2>${p(e)}</h2>`),n&&(i+=`<p>${p(n)}</p>`),i+="</div>",i}var g=`
...
`,u=class extends HTMLElement{#e;connectedCallback(){this.shadowRoot||this.attachShadow({mode:"open"}),this.#a(),this.render(this.shadowRoot),typeof this.liveLoad=="function"&&Promise.resolve(this.liveLoad()).catch(()=>{}),this.#e=new MutationObserver(()=>{let t=this.dataset.theme;this.#a(),this.dataset.theme!==t&&this.render(this.shadowRoot)}),this.#e.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}apiBase(){return(this.getAttribute("api-base")||"").trim()}disconnectedCallback(){this.#e?.disconnect()}#a(){let t=document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.dataset.theme=t;let e=(this.getAttribute("brand")||"").trim().toLowerCase();e==="assay"||e==="cai"||e==="watchdog"?this.dataset.brand=e:delete this.dataset.brand}json(t,e){let n=this.getAttribute(t);if(n==null||n.trim()==="")return e;try{return JSON.parse(n)}catch{return e}}};var s=[{label:"Critical",key:"critical",floor:0},{label:"Weak",key:"poor",floor:25},{label:"Adequate",key:"fair",floor:50},{label:"Strong",key:"healthy",floor:70},{label:"Exemplary",key:"exemplary",floor:90}];function x(a){return a>=90?s[4]:a>=70?s[3]:a>=50?s[2]:a>=25?s[1]:s[0]}function v(a){let t=Math.max(0,Math.min(100,a)),e=s.length-1;for(;e>0&&t<s[e].floor;)e--;let i=(e<s.length-1?s[e+1].floor:100)-s[e].floor,r=i>0?Math.max(0,Math.min(1,(t-s[e].floor)/i)):.5;return{leftPct:e*20+r*20,third:r<1/3?0:r<2/3?1:2,key:s[e].key}}function y(a,{variant:t="diamond",caps:e=!1,className:n}={}){let i=v(a),r=x(a),c=`${i.leftPct.toFixed(2)}%`,o=s.map(A=>`<i class="seg-${A.key}"></i>`).join(""),d;t==="diamond"?d=`<div class="cai-mk cai-diamond" style="left:${c};--dia:var(--band-${r.key})"><span class="cai-diamond-foot"></span></div>`:d=`<div class="cai-mk cai-pin" style="left:${c}"><span class="cai-pin-badge">${Math.round(a)}</span><span class="cai-pin-line"></span><span class="cai-pin-foot"></span></div>`;let f=e?'<div class="cai-caps"><span>Worst</span><span>Best</span></div>':"";return`<div class="${"cai-ladder"+(e?"":" compact")+(n?` ${n}`:"")}"><div class="cai-rail" role="img" aria-label="${Math.round(a)} of 100 on a fixed worst-to-best scale"><div class="cai-segs">${o}</div>${d}</div>${f}</div>`}var k=`
...
`;var w=new Map;function C(a,t,e){let n=(a||"").trim().replace(/\/$/,"");if(!n)return Promise.resolve(e);let i=n+" "+t,r=w.get(i);return r||(r=(async()=>{try{let c=await fetch(n+t);return c.ok?await c.json():e}catch{return e}})(),w.set(i,r),r)}async function S(a){let t=await C(a,"/api/oss",[]);return Array.isArray(t)?t:[]}function $(a){if(!Array.isArray(a)||a.length===0)return null;let t=n=>n.bestScore!=null?Number(n.bestScore):Number(n.headlineScore)||0,e=null;for(let n of a)(e===null||t(n)>t(e)||t(n)===t(e)&&(Number(n.productionLoc)||0)>(Number(e.productionLoc)||0))&&(e=n);return e}var N=m+h+g+k+`
...
`;customElements.define("cai-band-scale",class extends u{async liveLoad(){let a=this.apiBase();if(!a)return;let t=$(await S(a)),e=t&&(t.bestScore!=null?t.bestScore:t.headlineScore);e==null||!Number.isFinite(Number(e))||(this._liveScore=Number(e),this.render(this.shadowRoot))}render(a){let t=this.getAttribute("score"),e=this._liveScore!=null?this._liveScore:t!=null&&t!==""&&Number.isFinite(Number(t))?Number(t):null,n=e!=null,i=this.getAttribute("caption"),r;n?r=y(e,{variant:"pin"}):r=`<div class="cai-ladder compact"><div class="cai-rail" role="img" aria-label="The fixed worst-to-best CAI band scale"><div class="cai-segs">${s.map(f=>`<i class="seg-${f.key}"></i>`).join("")}</div></div></div>`;let c=s.map(d=>`<span class="ink-${d.key}">${d.label}</span>`).join(""),o=`<style>${N}</style>`;o+=b(this),o+='<figure class="mk-bandscale">',o+=`<div class="mk-bandscale-rail${n?" has-pin":""}">`,o+=r,o+='<div class="mk-bandscale-cuts" aria-hidden="true"><span style="left:0%">0</span><span style="left:20%">25</span><span style="left:40%">50</span><span style="left:60%">70</span><span style="left:80%">90</span><span style="left:100%">100</span></div>',o+="</div>",o+=`<div class="mk-bandscale-words">${c}</div>`,i&&(o+=`<figcaption class="mk-bandscale-cap">${p(i)}</figcaption>`),o+="</figure>",a.innerHTML=o}});
