...
`;function o(n){return String(n??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function f(n){if(n==null||n==="")return"";let t=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,e="",r=0,a;for(;(a=t.exec(n))!==null;){a.index>r&&(e+=o(n.slice(r,a.index)));let s=a[0];if(s.startsWith("**"))e+=`<strong>${o(s.slice(2,-2))}</strong>`;else if(s.startsWith("`"))e+=`<code>${o(s.slice(1,-1))}</code>`;else{let d=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(s);d?e+=`<a href="${o(d[2])}">${o(d[1])}</a>`:e+=o(s)}r=a.index+s.length}return r<n.length&&(e+=o(n.slice(r))),e}var b=`
...
`;function g(n){let t=n.getAttribute("kicker"),e=n.getAttribute("heading"),r=n.getAttribute("lede");if(!t&&!e&&!r)return"";let a='<div class="mk-section-head">';return t&&(a+=`<span class="mk-kicker">${o(t)}</span>`),e&&(a+=`<h2>${f(e)}</h2>`),r&&(a+=`<p>${f(r)}</p>`),a+="</div>",a}var x=`
...
`;customElements.define("cai-c4-heat",class extends u{async liveLoad(){let n=this.apiBase();if(!n)return;this._pending=!0,this.render(this.shadowRoot);let t=this.getAttribute("owner")||"",e=this.getAttribute("name")||"";try{let r=await h(n,"/api/public/c4",null),a=r&&Array.isArray(r.items)?r.items:[];if(a.length===0)return;let s=await h(n,"/api/oss",null),d=new Map;if(Array.isArray(s))for(let l of s){let c=l.bestRunId||l.BestRunId;c&&d.set(String(c),l)}let i=null;for(let l of a){let c=d.get(String(l.runId));if(c)if(t&&e){if(c.owner===t&&c.name===e){i=c;break}}else i||(i=c)}if(!i)return;let m=await k(n,"/api/public/oss/"+encodeURIComponent(i.owner)+"/"+encodeURIComponent(i.name)+"/c4.svg");if(!m)return;this._live={owner:i.owner,name:i.name,svg:m}}finally{this._pending=!1,this.render(this.shadowRoot)}}render(n){let t=`<style>${v}</style>`;if(t+=g(this),!this._live){this._pending&&(t+='<figure class="mk-c4"><div class="mk-c4-frame"><p class="mk-c4-loading">Loading the architecture map\u2026</p></div></figure>'),n.innerHTML=t;return}let{owner:e,name:r,svg:a}=this._live,s=this.getAttribute("caption");t+='<figure class="mk-c4">',t+='<div class="mk-c4-frame">',t+=`<div class="mk-c4-repo"><strong>${o(r)}</strong><span>by ${o(e)}</span></div>`,t+=a,t+="</div>",s&&(t+=`<figcaption class="mk-c4-cap">${f(s)}</figcaption>`),t+="</figure>",n.innerHTML=t}});
