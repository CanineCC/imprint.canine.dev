...
`;function i(n){return String(n??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function c(n){if(n==null||n==="")return"";let e=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,t="",a=0,r;for(;(r=e.exec(n))!==null;){r.index>a&&(t+=i(n.slice(a,r.index)));let s=r[0];if(s.startsWith("**"))t+=`<strong>${i(s.slice(2,-2))}</strong>`;else if(s.startsWith("`"))t+=`<code>${i(s.slice(1,-1))}</code>`;else{let o=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(s);o?t+=`<a href="${i(o[2])}">${i(o[1])}</a>`:t+=i(s)}a=r.index+s.length}return a<n.length&&(t+=i(n.slice(a))),t}var f=`
...
`;function u(n){let e=n.getAttribute("kicker"),t=n.getAttribute("heading"),a=n.getAttribute("lede");if(!e&&!t&&!a)return"";let r='<div class="mk-section-head">';return e&&(r+=`<span class="mk-kicker">${i(e)}</span>`),t&&(r+=`<h2>${c(t)}</h2>`),a&&(r+=`<p>${c(a)}</p>`),r+="</div>",r}var p=`
...
`;function v(n){let e=String(n||""),t=e.indexOf("/");return t<=0||t>=e.length-1?null:{owner:e.slice(0,t),name:e.slice(t+1)}}customElements.define("cai-c4-heat",class extends l{#e=[];#t=0;#n=new Map;async liveLoad(){let n=this.apiBase();if(!n)return;this._pending=!0,this.render(this.shadowRoot);let e=await g(n);if(this.#e=e.map(t=>{let a=t&&t.owner,r=t&&t.name,s=a&&r?{owner:a,name:r}:v(t&&t.repo);return s?{repo:t&&t.repo,owner:s.owner,name:s.name}:null}).filter(Boolean),this._pending=!1,this.#e.length===0){this.render(this.shadowRoot);return}this.#t=0,await this.#a(0),this.render(this.shadowRoot)}async#a(n){if(this.#n.has(n))return;let e=this.#e[n],t=await b(this.apiBase(),"/api/public/oss/"+encodeURIComponent(e.owner)+"/"+encodeURIComponent(e.name)+"/c4.svg");this.#n.set(n,t||"")}async#r(n){this.#e.length<2||(this.#t=(this.#t+n+this.#e.length)%this.#e.length,await this.#a(this.#t),this.render(this.shadowRoot))}render(n){let e=`<style>${k}</style>`;if(e+=u(this),this.#e.length===0){this._pending&&(e+='<figure class="mk-c4"><div class="mk-c4-frame"><p class="mk-c4-loading">Loading the architecture maps\u2026</p></div></figure>'),n.innerHTML=e;return}let t=this.#e[this.#t],a=this.#n.get(this.#t),r=this.getAttribute("caption"),s=this.#e.length<2;e+='<figure class="mk-c4">',e+='<div class="mk-c4-bar">',e+=t.repo&&t.repo.indexOf("/")>=0?`<span class="mk-c4-repo"><strong>${i(t.name)}</strong><span>by ${i(t.owner)}</span></span>`:`<span class="mk-c4-repo"><strong>${i(t.repo||t.name)}</strong></span>`,s||(e+='<span class="mk-c4-nav">',e+='<button type="button" class="mk-c4-btn" data-c4-prev aria-label="Previous architecture map">\u2039</button>',e+=`<span class="mk-c4-count">${this.#t+1} / ${this.#e.length}</span>`,e+='<button type="button" class="mk-c4-btn" data-c4-next aria-label="Next architecture map">\u203A</button>',e+="</span>"),e+="</div>",e+='<div class="mk-c4-frame">',a===void 0?e+='<p class="mk-c4-loading">Loading the architecture map\u2026</p>':a===""?e+=`<p class="mk-c4-loading">Couldn't load this architecture map \u2014 try the next one.</p>`:e+=a,e+="</div>",r&&(e+=`<figcaption class="mk-c4-cap">${c(r)}</figcaption>`),e+="</figure>",n.innerHTML=e;let o=n.querySelector("[data-c4-prev]"),d=n.querySelector("[data-c4-next]");o&&o.addEventListener("click",()=>{this.#r(-1)}),d&&d.addEventListener("click",()=>{this.#r(1)})}});
