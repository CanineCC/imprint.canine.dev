...
`;function s(n){return String(n??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function d(n){if(n==null||n==="")return"";let t=/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g,e="",r=0,a;for(;(a=t.exec(n))!==null;){a.index>r&&(e+=s(n.slice(r,a.index)));let o=a[0];if(o.startsWith("**"))e+=`<strong>${s(o.slice(2,-2))}</strong>`;else if(o.startsWith("`"))e+=`<code>${s(o.slice(1,-1))}</code>`;else{let l=/^\[([^\]]+)\]\(([^)]+)\)$/.exec(o);l?e+=`<a href="${s(l[2])}">${s(l[1])}</a>`:e+=s(o)}r=a.index+o.length}return r<n.length&&(e+=s(n.slice(r))),e}var b=`
...
`;function g(n){let t=n.getAttribute("kicker"),e=n.getAttribute("heading"),r=n.getAttribute("lede");if(!t&&!e&&!r)return"";let a='<div class="mk-section-head">';return t&&(a+=`<span class="mk-kicker">${s(t)}</span>`),e&&(a+=`<h2>${d(e)}</h2>`),r&&(a+=`<p>${d(r)}</p>`),a+="</div>",a}var x=`
...
`;customElements.define("cai-c4-heat",class extends f{async liveLoad(){let n=this.apiBase();if(!n)return;this._pending=!0,this.render(this.shadowRoot);let t=this.getAttribute("owner")||"",e=this.getAttribute("name")||"";try{let r=await u(n,"/api/public/c4",null),a=r&&Array.isArray(r.items)?r.items:[];if(a.length===0)return;let o=await u(n,"/api/oss",null),l=new Set;if(Array.isArray(o))for(let i of o)k(i)&&l.add(i.owner+"/"+i.name);let h=a.map(i=>v(i.repo)).filter(Boolean),c=null;if(t&&e?c=h.find(i=>i.owner===t&&i.name===e)||null:c=h.find(i=>l.has(i.owner+"/"+i.name))||null,!c)return;let m=await w(n,"/api/public/oss/"+encodeURIComponent(c.owner)+"/"+encodeURIComponent(c.name)+"/c4.svg");if(!m)return;this._live={owner:c.owner,name:c.name,svg:m}}finally{this._pending=!1,this.render(this.shadowRoot)}}render(n){let t=`<style>${y}</style>`;if(t+=g(this),!this._live){this._pending&&(t+='<figure class="mk-c4"><div class="mk-c4-frame"><p class="mk-c4-loading">Loading the architecture map\u2026</p></div></figure>'),n.innerHTML=t;return}let{owner:e,name:r,svg:a}=this._live,o=this.getAttribute("caption");t+='<figure class="mk-c4">',t+='<div class="mk-c4-frame">',t+=`<div class="mk-c4-repo"><strong>${s(r)}</strong><span>by ${s(e)}</span></div>`,t+=a,t+="</div>",o&&(t+=`<figcaption class="mk-c4-cap">${d(o)}</figcaption>`),t+="</figure>",n.innerHTML=t}});
