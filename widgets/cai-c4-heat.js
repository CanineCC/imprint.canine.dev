...
`;function v(n){let e=String(n||""),t=e.indexOf("/");return t<=0||t>=e.length-1?null:{owner:e.slice(0,t),name:e.slice(t+1)}}customElements.define("cai-c4-heat",class extends l{#e=[];#t=0;#n=new Map;async liveLoad(){let n=this.apiBase();if(!n)return;this._pending=!0,this.render(this.shadowRoot);let e=await b(n);if(this.#e=e.map(t=>{let r=v(t&&t.repo);return r?{repo:t.repo,owner:r.owner,name:r.name}:null}).filter(Boolean),this._pending=!1,this.#e.length===0){this.render(this.shadowRoot);return}this.#t=0,await this.#r(0),this.render(this.shadowRoot)}async#r(n){if(this.#n.has(n))return;let e=this.#e[n],t=await g(this.apiBase(),"/api/public/oss/"+encodeURIComponent(e.owner)+"/"+encodeURIComponent(e.name)+"/c4.svg");this.#n.set(n,t||"")}async#a(n){this.#e.length<2||(this.#t=(this.#t+n+this.#e.length)%this.#e.length,await this.#r(this.#t),this.render(this.shadowRoot))}render(n){let e=`<style>${k}</style>`;if(e+=u(this),this.#e.length===0){this._pending&&(e+='<figure class="mk-c4"><div class="mk-c4-frame"><p class="mk-c4-loading">Loading the architecture maps\u2026</p></div></figure>'),n.innerHTML=e;return}let t=this.#e[this.#t],r=this.#n.get(this.#t),a=this.getAttribute("caption"),s=this.#e.length<2;e+='<figure class="mk-c4">',e+='<div class="mk-c4-bar">',e+=`<span class="mk-c4-repo"><strong>${i(t.name)}</strong><span>by ${i(t.owner)}</span></span>`,s||(e+='<span class="mk-c4-nav">',e+='<button type="button" class="mk-c4-btn" data-c4-prev aria-label="Previous architecture map">\u2039</button>',e+=`<span class="mk-c4-count">${this.#t+1} / ${this.#e.length}</span>`,e+='<button type="button" class="mk-c4-btn" data-c4-next aria-label="Next architecture map">\u203A</button>',e+="</span>"),e+="</div>",e+='<div class="mk-c4-frame">',r===void 0?e+='<p class="mk-c4-loading">Loading the architecture map\u2026</p>':r===""?e+=`<p class="mk-c4-loading">Couldn't load this architecture map \u2014 try the next one.</p>`:e+=r,e+="</div>",a&&(e+=`<figcaption class="mk-c4-cap">${c(a)}</figcaption>`),e+="</figure>",n.innerHTML=e;let o=n.querySelector("[data-c4-prev]"),d=n.querySelector("[data-c4-next]");o&&o.addEventListener("click",()=>{this.#a(-1)}),d&&d.addEventListener("click",()=>{this.#a(1)})}});
